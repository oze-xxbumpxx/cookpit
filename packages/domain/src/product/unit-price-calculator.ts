...
export class UnitPriceCalculator {
  static calculate(price: Money, packageSize: Quantity): Money {
    ...
    if (WEIGHT_UNITS.has(unit)) {
      ...
      unitPriceAmount = (price.amount / gramValue) * 100;
    } else if (VOLUME_UNITS.has(unit)) {
      const mlValue = unit === 'l' ? value * 1000 : value;
      unitPriceAmount = (price.amount / mlValue) * 100;
    } else {
      unitPriceAmount = price.amount / value;
    }

    const rounded = Math.round(unitPriceAmount * 10) / 10;
    return Money.of(rounded, price.currency);
  }
}
