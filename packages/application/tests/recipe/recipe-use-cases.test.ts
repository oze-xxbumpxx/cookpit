...
describe('CreateRecipeUseCase', () => {
  ...
  it('レシピを保存し、入力どおりの DTO を返す（mapper の往復を含む）', async () => {
    const useCase = new CreateRecipeUseCase(repository, passthroughUnitOfWork);
  ...
  it('生成した ID で後から取得できる', async () => {
    const created = await new CreateRecipeUseCase(repository, passthroughUnitOfWork).execute(
      baseInput,
    );
  ...
  it('servings: 4 を指定すると DTO の servings === 4 になる (T-A01)', async () => {
    const dto = await new CreateRecipeUseCase(repository, passthroughUnitOfWork).execute({
      ...baseInput,
      servings: 4,
    });
  ...
  it('servings を省略すると DTO の servings === null になる (T-A02)', async () => {
    const dto = await new CreateRecipeUseCase(repository, passthroughUnitOfWork).execute(baseInput);
...
describe('UpdateRecipeUseCase', () => {
  ...
  it('編集可能フィールドを更新し、baseServings は変更しない', async () => {
    ...
    const dto = await new UpdateRecipeUseCase(repository, passthroughUnitOfWork).execute(
      updateInput,
    );
  ...
  it('ドメインバリデーション違反は保存されない (UC-GAP-3)', async () => {
    ...
    await expect(
      new UpdateRecipeUseCase(repository, passthroughUnitOfWork).execute({
        ...updateInput,
        name: '',
      }),
...
describe('DeleteRecipeUseCase', () => {
  it('存在するレシピを削除する', async () => {
    ...
    await new DeleteRecipeUseCase(repository, passthroughUnitOfWork).execute('id-1');
  ...
  it('存在しない ID は RecipeNotFoundError を投げ、delete を呼ばない', async () => {
    await expect(
      new DeleteRecipeUseCase(repository, passthroughUnitOfWork).execute('missing'),
    ).rejects.toBeInstanceOf(RecipeNotFoundError);
  ...
  it('冪等性: 削除済みの ID を再度削除すると NotFound になる', async () => {
    repository.seed(seededRecipe('id-1'));
    const useCase = new DeleteRecipeUseCase(repository, passthroughUnitOfWork);
