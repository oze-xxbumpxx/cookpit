...
import { getDb, getTxConnection } from '@/db/client';
import {
 ...
 * WebSocket 接続（`getTxConnection`）の上で開く。読み取りは neon-http のまま
 * （設計書「トランザクション再導入の設計案」案 S）。
 *
 * 接続は `execute` のたびに張られ、`execute` の `finally` で閉じられる（ADR-0020）。
 */
