...
## 対応状況（2026-08-04 レビュー後対応）

対応コミット: `fix(shopping-lists): 価格比較のレビュー指摘 M-1/S-1/S-3/S-4 を修正する`

| 指摘     | 対応         | 内容                                                                                                                                                                                            |
| -------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M-1      | **修正**     | `buildStoreUnitPriceBreakdown` に `latestRecords.length < 2` ガードを追加。S-3 の対応で `findCheapestRecord`（初期値なし `reduce`）自体が不要になり、欠陥経路は消滅した。回帰は PC-20b / PC-20c |
| S-1      | **修正**     | 丸め後の差が 0 になる非最安行は「ほぼ同額」表記に変更（ユーザー確定）。`formatStoreUnitPriceDiffLabel` を新設し PC-34 / PC-35 で固定                                                            |
| S-2      | **現状維持** | ユーザー判断。`basisLabel` の `1` 前置と `other` の生文字列一致は設計書 §計算仕様「既知の限定事項 2」へ明記。PC-32 が `11L` 表示を意図的な既知挙動として固定                                    |
| S-3      | **修正**     | 基準区分を「記録数が最も多い kind（同数は weight → volume → other）」に変更。次元をまたぐ `unitPriceAmount` の数値比較を廃止。PC-32 / PC-33 で固定                                              |
| S-4      | **修正**     | `page.tsx` で `shoppingList.items` の `productId` 集合に絞ってから Client Component へ渡す。設計書 §データフロー を更新                                                                         |
| S-5      | **現状維持** | ユーザー判断。価格取得の失敗を縮退させない理由（縮退すると「価格記録が無い品目」と区別がつかない）を設計書 §エラー処理 へ明記                                                                   |
| S-6      | **修正**     | LC-33（店舗再割当が価格計算に影響しない）/ LC-34（フォーカス復帰 refetch が products を再取得しない）を実装                                                                                     |
| S-7      | **実施**     | MB-01〜06 を 390px で実施し全 PASS（下記）                                                                                                                                                      |
| N-1      | **修正**     | 公開 4 関数（+ 新設 1）へ JSDoc を追加。`diffFromCheapestYen` / `basisLabel` のフィールドコメントも復活                                                                                         |
| N-2〜N-8 | 申し送り     | 今スプリント内では対応しない。N-7（完了時に `expandedItemId` がリセットされない）は既存の穴で本 PR による悪化なし                                                                               |

`apps/web` のテストは 700 → 708 件。`pnpm lint`（0 error）/ `pnpm type-check` / `pnpm test`（全 5 パッケージ 1659 件）green。

### S-7: 実画面確認（MB-01〜06。390px / Chromium / PGlite dev サーバー）

確認データ: 献立「肉じゃが」から生成した買い物リスト。玉ねぎ（価格記録 2 店舗 = 比較成立）/
豚こま切れ肉（1 店舗のみ = 縮退）/ じゃがいも（`productId` が `null` = 対象外）の 3 品目。

| ID    | 判定 | 根拠                                                                                                                                                                 |
| ----- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MB-01 | PASS | 「業務スーパーBの方が約10円安い」が幅 142px で収まり、横スクロール 0px・見切れなし。**ただし本文列の幅の都合で 2 行に折り返す**（見切れではないため PASS。下記所見） |
| MB-02 | PASS | 玉ねぎの内訳を開いた状態でじゃがいもを展開すると、玉ねぎ側のパネルが閉じる（`expandedItemId` の既存仕様どおり同時に 1 行のみ）                                       |
| MB-03 | PASS | 内訳 2 行「業務スーパーB 42.7円 / 1個 ← 最安」「スーパーA 52.7円 / 1個 +10円」が折り返さず表示。横スクロール 0px                                                     |
| MB-04 | PASS | 玉ねぎを bought にして展開。価格入力 147×44px / 実購入店舗セレクト 147×44px が潰れず、内訳セクションはボタン行より下に区切り線付きで表示。横スクロール 0px           |
| MB-05 | PASS | 3 品目すべてチェックボタンの左端が揃い（差 <1px）、タップ領域 ≥24px。総額差行・展開トリガーの追加による横方向の崩れなし                                              |
| MB-06 | PASS | じゃがいも（`productId` が `null`）は総額差行 0・展開トリガー 0 で従来どおりのレイアウト。豚こま切れ肉（価格記録 1 店舗のみ）も展開トリガー 0 で縮退                 |

計算の実データ突き合わせ: 玉ねぎの単価は スーパーA 52.7 円/個・業務スーパーB 42.7 円/個 で、
必要量 1 個・`kind === 'other'`（`canonicalFactor=1` / `divisor=1`）。総額差 `round(52.7 - 42.7) = 10` 円、
内訳の差額 `+10円` が実画面と一致した（設計書 §計算仕様どおり）。

**所見（FAIL ではないが記録）**: MB-01 の総額差行は 390px の本文列で 2 行に折り返す
（「業務スーパーBの方が約10 / 円安い」）。試験計画の PASS 基準は「横スクロール・文字の見切れが
発生しない」なので基準は満たすが、項目名の「折り返しなく収まる」は満たしていない。
店舗名が長いほど折り返しやすく、`w-28` の店舗バッジと本文列で幅を分け合う構造上の帰結である。
表示上の実害は無いため本タスクでは変更しないが、店舗名の省略や文言短縮を検討する余地がある。
