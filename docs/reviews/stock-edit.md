...
# 追補: MB-11〜14（買い物完了パネル）の確認 — BLOCKED 解消

- 実施日: 2026-08-09
- **結果: MB-11〜14 すべて PASS。実画面確認の BLOCKED は 0 件になった。**

初回の確認では「dev シードに献立・買い物リストが無くパネルへ到達できない」として BLOCKED
にしていたが、`tests/e2e/saturday-flow.spec.ts` が同じ経路を通っていたため到達可能だった。
**「到達手段が無い」ではなく「到達手段を探していなかった」**のが実態で、
BLOCKED の判定自体が甘かった。

## MB-12 / MB-13: E2E へ恒久的な回帰ガードとして追加

`apps/web/tests/e2e/saturday-flow.spec.ts` を拡張した（新規 spec を足すと CI 分数を
消費するため、既存の土曜フローに組み込む。`ci-minutes-reduction.md` の制約）。

- 買い物完了パネルで「賞味期限を設定」→ 日付入力 → 完了
- `/pantry` で**その期限が在庫に載っている**ことを確認（`〜12/24まで`）

日付は**固定日 `2030-12-24`** を使い「今日」に依存させない（CI が日付境界をまたいでも
揺れないようにする。実際に 00:37 UTC の実行で E2E がフレークした直後だったため）。

**Red の確認済み**: パネルの `expiresAt` を `null` 固定へ戻すと、この行で正しく落ちる。

```
✘ saturday-flow.spec.ts:68 › 献立から買い物リストを作り、購入品を在庫化して消費できる
  > 125 | await expect(stockRow.getByText(STOCK_EXPIRES_AT_LABEL, ...)).toBeVisible();
```

これが **roadmap Sprint 8 完了条件「買い物完了時に賞味期限を入力できる（任意）」の
end-to-end の裏取り**になる。同時に **Unit B（賞味期限アラート）の前提**
（主要な流入経路が期限データを生む）を CI で守り続ける。

## MB-11 / MB-14: 実画面で確認（`scripts/verify-complete-panel.mjs`）

E2E はデスクトップ幅で走るため、展開 UI の見え方と 375px のレイアウトは別途確認した。
スクリプトは献立 → 買い物リストの用意まで自分で行うので事前シード不要。

| ID    | 判定 | 内容                                                                                                                                                                                                                                                        |
| ----- | ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MB-11 | PASS | 既定は入力欄 0 個・`aria-expanded=false`（パネル高 **320px**）→ 展開で入力欄 1 個・`aria-expanded=true`・文言が「賞味期限を削除」へ（**392px**）。**Q-1 の「パネルが縦に長くなる」懸念どおり、既定状態の高さは変わらない**（P-3 案 B の狙いが実画面で成立） |
| MB-14 | PASS | 375px で横スクロールなし（`scrollW=375 / clientW=375`）・要素のはみ出しなし・**日付入力は数量/保存場所の横並びに入らず独立行**（P-3 で排除した案 A になっていないことを座標で確認）                                                                         |

## 実画面確認の最終結果（Unit A 全体）
...
PGlite の dev DB が今回も複数回 `RuntimeError: Aborted()` で壊れた。原因は
**停止しきっていない `next dev` が古い `.pglite-dev` を掴んだまま新しい DB を作った**こと。
`pkill -f "next dev"` は**自分のシェル（コマンド文字列に "next dev" を含む）を巻き込んで
落とす**ため機能しない。`/proc/<pid>/cmdline` で `next-server` を探して kill し、
ポートが空いたことを確認してから作り直す必要がある。
`docs/07-dev-rules.md` §環境の既知の事実 への追記候補。
