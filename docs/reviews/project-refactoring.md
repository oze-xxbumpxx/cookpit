...
判断を要する項目が 2 件（S1 / S2）ある。いずれも並行操作・過渡表示に限られ、
データ整合性やエラー変換には影響しない。
...
- 判断: **ユーザー判断事項**。受容するか、pending を単一キーから集合（`Set<string>`）に変えるか。
  集合化すれば変更前の「独立した pending」と等価になり、N1 も同時に解消する。
...
- 影響: 過渡表示のみ。既存テスト（LC-21「refetch 成功で既存のエラーバナーがクリアされる」）は
  どちらの挙動でも通る。
- 判断: **ユーザー判断事項**。「新しい試行を始めたら前のエラーを消す」は一般的な挙動であり
  受容が妥当と考えるが、厳密に変更前と揃えるなら開始時クリアを抑止するオプションが要る。

## Nice

### N1: `pantry-client` が `submittingStockId` に `'add'` / `'refresh'` を取り得る値を渡している

`submittingStockId={action.pendingKey}` は、stockId が文字列 `'add'` / `'refresh'` と
衝突しない前提（実際は UUID のため安全）に暗黙依存している。現状は行の disable は壊れないが、
子側で `isPending(stock.id)` を判定する形のほうが堅牢。S1 を集合化で解決する場合は同時に解消できる。
