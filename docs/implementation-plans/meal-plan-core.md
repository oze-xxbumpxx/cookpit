...
| E-6 | `packages/api-contract/package.json` | `devDependencies` に `vitest` 追加、`scripts.test` に `"vitest run"` 追加（G-3） | 既存 domain/application/infrastructure package.json と同一パターン |
...
| R-5（申し送り・対応不要） | 非土曜日の `weekIdentifier`（例 `"2026-07-05"` 日曜）を `POST /api/meal-plans` に渡しても Zod（`z.iso.date()`）も Domain（`WeekIdentifier.fromString`）も曜日検証をしないため拒否されない（設計 §19-11 契約側申し送り） | 実際の週開始日と異なる `weekStartDate` を持つ MealPlan が作成され得る | **MVP1 では対応しない**（設計 §19-11 の通り、Zod/Domain いずれの契約違反でもない）。実装時に顕在化した場合は独断で Domain に曜日検証を追加せず、Orchestrator 経由で architecture-designer に差し戻す |
| R-6（申し送り・対応不要） | `scaleFactor` に `Infinity` を渡すと `z.number().positive()` は `Infinity > 0` が真のため accept してしまう（設計 §19-11） | 実用上無意味な値が保存され得るが契約違反ではない | **MVP1 では対応しない**。test-designer が境界値として言及する可能性があるが、Zod スキーマ・Domain バリデーションともに変更しない |
