...
- 完了条件: 上記ケースすべてが green（PR #202 レビュー反映後で計 47 ケース。試験計画の
  MW-01〜MW-23 / MW-18b / MW-19b / MW-19c / MW-M01〜MW-M12 に対応）。`vitest run --project node`
...
3. **単体テスト追加** … `apps/web/tests/proxy.node.test.ts` を新規作成し、上記ケース（計 47）を
   実装する。完了: 新規テストのみ実行して green（`pnpm --filter @cookpit/web test -- proxy`
   等で対象を絞って先に確認してよい）。
...
| R-3 | matcher は通っても実際の対象/除外パス判定が意図と異なる              | 単体テストの構造テスト（MW-M01〜MW-M12）+ 本番相当サーバでの black-box 確認 | 手動確認で不一致が見つかった場合は matcher の否定 lookahead パターンを見直し、再度ビルド・手動確認をやり直す                                                                                                                                                                                                                                     |
