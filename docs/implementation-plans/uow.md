...
**有効化完了（2026-08-16）**: Preview 検証の誤判定と本番障害を経て、ADR-0020 / PR #173 / #174 で
接続寿命と `ws` バンドルを修正した。本番は `DB_WRITE_TRANSACTION=on`。U-2 のレイテンシ実測だけは
未了のまま持ち越す。
...
| 4   | 本番へ同じ環境変数を設定して再デプロイ                                | **完了・PASS**（2026-08-16・PR #174 後）  |

事故時は環境変数を落として再デプロイするだけで戻る（PR 不要）。
**Sprint 10 完了条件 2 は手順 4 と本番書き込み確認をもって達成済み。**
