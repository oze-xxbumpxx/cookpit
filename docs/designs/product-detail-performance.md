...
1. **`GET /api/products/:id/cheapest-store` と `GET /api/products/:id` の扱い**:
   ...
   （削除するなら別タスクとして切り出す）。
2. **案1（新規 UseCase）で進めてよいか**: 案2（`GetProductUseCase` 拡張）もクエリコストは
   同等のため、影響範囲の小ささよりコード量の少なさを優先するなら案2もあり得る。
   推奨は案1だが、最終選択はユーザー確認を経て確定する。
3. **`ssr: false` のちらつきの許容可否**: 初回表示でスケルトン → チャートに切り替わる
   見た目の変化を許容できるか、実装後の目視確認で判断する（許容できなければ `ssr: true`
   ＋ フォールバックなしの構成に変更する）。
