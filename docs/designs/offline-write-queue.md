...
- Background Sync API（`SyncManager`）の採用。理由・未確認の前提は §リスク R-1 を参照。
...
export async function enqueueCheckedOp(op: Omit<QueuedCheckedOp, 'enqueuedAt' | 'attempts'>): Promise<void>;
...
| ID | リスク | 影響 | 対策 |
| --- | --- | --- | --- |
| R-1 | iOS Safari の Background Sync 未対応という前提が一次情報で未確認 | もし対応していた場合、より堅牢な再送機構（`sw.ts` 経由）を見送ったまま実装することになる | 実装着手前に Web 検索可能な環境で一次情報を確認することを推奨（要件書§未決事項1）。誤りが判明しても本設計のアプリ層キューは無駄にならず、Background Sync は追加の補強として後から積める |
...
4. **P-7 の TTL（24時間）・attempts 上限（5回）の具体値**。設計上の推奨値であり、実運用での
   ...
   推奨する。
5. **R-1: Background Sync の iOS 対応状況の一次情報確認**。
