...
| R-e | **名前空間が cwd に依存し、別ディレクトリへ静かに書く**（下記で対応済み） | PR #129 前のリスク確認 | `.git` を遡ってリポジトリ境界を特定 + 既定値へ落ちたら警告            |

### R-e: 定数を導出へ変えたことで生まれた退行（2026-07-31 に検出・修正）

**PR #129 のマージ可否を問われて実測し、発見した。** 変更前の `STATE_NAMESPACE` は定数で
あり cwd に依存しなかったが、`package.json` から導出する形にしたことで
**`CLAUDE_PROJECT_DIR` 未設定かつ cwd がリポジトリ外だと `claude-harness` へ落ち、
別の状態ディレクトリへ書く**ようになっていた。`trusted: true` かつ警告なしで、
**完全に静かな失敗**だった（実測: `/tmp` から `harness-run.mjs where` が
`/root/.local/state/claude-harness` を返した）。

対応は 2 点。

1. **`.git` を持つ最も近い祖先を探してから `package.json` を読む**（`repoRootFromMarker`）。
   モノレポのサブディレクトリから実行しても workspace 側を拾わない
   （実測: `apps/web`（`@cookpit/web`）から解決しても `cookpit-harness`）。
2. **既定値へ落ちたら `warnings` に積む**。プロジェクトを特定できない状況は
   推測で解決せず、利用者に見せる。

**モジュール自身の位置（`import.meta.url`）から遡る案は却下した。** Phase 3 で Plugin として
配布するとモジュールは利用者リポジトリの外に置かれ、全プロジェクトが同一の名前空間に
なってしまう（cwd 依存より悪化する）。
