...
実装は `isCountableUnit` を**使わない**。上表のとおり「`g` / `ml` だけ 300、それ以外はすべて 1」
という単一の規則で必要な出し分けがすべて成立し、可算判定を挟む必要が無いためである
（自由入力の未知単位も安全側の 1 に落ちる）。Domain を値として import しない理由は
`store-name.ts` と同じ（`node:crypto` の混入）。

> **バリデーションエラー文は UI から到達しない。**`canSubmit` が
> `parsedPackageSize.kind === 'amount' && value > 0` を要求するため、内容量が不正なまま
> 送信できず `errors.packageSizeValue` の分岐に入らない（`handleSubmit` が `canSubmit` で
> 早期 return する）。本設計では文言の一致（R-8）を `packageSizeExample` の共有によって
> 担保し、到達不能な分岐はそのまま残す（防御的コードとして意味があり、削るのはスコープ外）。
> この事実は §未決事項 に記録する。
...
| 生成物         | `apps/web/src/fonts/*.woff2` + 生成 CSS `src/app/fonts.css` を**コミット**         |
| ビルド手順     | 増やさない（生成は手動スクリプト、成果物はリポジトリに入れる）                     |
| 実測結果       | 4.40 MB → **1.89 MB（-57%）**。見込みの 0.5〜0.9MB には届かなかった（下記）        |

漢字リストを手打ちしない理由: 第一水準は 2965 字あり、手打ちでは取りこぼしを検証できない。
区点 → Shift_JIS の変換式で機械生成すれば網羅性が構造的に保証される。Node 20+ は full ICU を
同梱するため `TextDecoder('shift_jis')` が使える。スクリプトは先頭（区 16 点 1 = `亜` =
`0x889F`）と末尾（区 47 点 51 = `腕` = `0x9872`）を assert し、さらに生成数が規格値 2965 と
一致することを確認して変換式の正しさを自己検証する。

生成物を `public/` ではなく `src/` に置くのは、webpack にハッシュ付きで
`/_next/static/media/` へ出させて `immutable` キャッシュを効かせるため（`public/` 配下は
Vercel の既定で `must-revalidate` になり、@fontsource 時代のキャッシュ特性が失われる）。

**見込みを外した理由と、さらに削る場合の選択肢**: 丸ゴシックのグリフは 1 ウェイト約 630KB
あり、第一水準 2965 字を収録すると 3 ウェイトで 1.89MB になる。追加で削るには
(a) ウェイトを 400/700 の 2 種にする（-33%。`font-medium` が通常太さになる）、
(b) 収録字数を常用漢字 2136 字へ削る（-28%。食材名の第一水準漢字が fallback に落ちる）
のいずれかが必要で、どちらも見た目・運用の判断を伴うため本タスクでは実施しない。

**unicode-range 分割は実装して実測したうえで却下した。** 実際の画面テキストで 16 スライス
全部に到達し、転送量は単一サブセットと変わらないまま CSS が 73KB 増えた。Google Fonts の
日本語分割が効くのはグリフを**頻度順**に並べているためで、JIS（読み）順のスライスでは
漢字がコードポイント全域へ散る。信頼できる頻度表を持ち込めないため単一サブセットに留める。
...
| 内容量バリデーションの到達不能分岐                      | ユーザー | `canSubmit` が先に送信を止めるため `errors.packageSizeValue` は画面に出ない。文言は統一済みで実害は無いが、防御的コードとして残すか整理するかは別タスク |
