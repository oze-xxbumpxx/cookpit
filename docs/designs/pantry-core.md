...
| (2) の途中/直後   | status は active のまま → 全段を再実行。Stock はスキップされるが、**記録済みの価格が重複记録される窓がある**（後述） | ✔（価格重複の許容つき） |
| (3) の直後        | completed → 冪等パスへ。MealPlan 遷移のみ修復して成功を返す                                                          | ✔    |
| (4) の失敗        | 同上（冪等パスの修復で収束）                                                                                          | ✔    |
...
| B   | `stocks` のみ防止し、価格の重複窓（ステップ 3 の保存だけが失敗した場合の再実行）は許容    | **推奨**。窓は「(2) 完了後・(3) の単一 UPDATE 失敗時」のみと極小。重複しても `cheapestStoreAt` は店舗ごと最新 1 件を見るため判定は変わらず、`averagePrice` がわずかに歪む程度。既存集約に手を入れない |
...
統計精度」であり、致命側（Stock 重複）だけを DB 保証し、軽微側（価格重複）は狭い窓として
リスク登録する（§リスク R-2）。案 A はユーザーが完全収束を求める場合の代替として明記する。

#### (5) ADR 化の推奨

**ADR 化を推奨する（可）**。「買い物完了の冪等性・4 段保存順序・`source_shopping_item_id`
UNIQUE による Stock 二重追加防止・価格重複窓の許容」は ADR-0006 と対をなす恒久判断であり、
...
- **(d) 部分失敗**: 4 段保存を非トランザクションで行う（現行構成の踏襲・ADR-0006 と同じ）。
  §S-3 (2) の表のとおり全失敗点が再実行で収束する。唯一の非収束的副作用は価格記録の重複窓
  （S-3 (4) 案 B で許容・R-2）。トランザクション導入は引き続き全 Repository 横断の将来課題
...
| R-2 | 価格記録の重複窓（S-3 (4) 案 B。complete 保存だけが失敗した後の再実行で価格が二重記録され得る）                          | 窓が極小・影響軽微（cheapestStoreAt は不変・averagePrice が微小に歪む）として許容。完全収束が必要なら案 A へ    |
...
1. **買い物完了の冪等性・4 段保存順序・`source_shopping_item_id` UNIQUE による Stock 二重
   追加防止・価格重複窓の許容**（S-3）— ADR-0006 と対をなす恒久判断。**ADR 化を推奨**
   （仮番号 ADR-0007）
...
- **S-1〜S-11**（§設計判断リスト）。特に S-1/S-2（Repository IF と DB の根幹）・
  S-3（API の冪等セマンティクス）・S-10（API 入力形状 = reason の有無）は contract-designer /
  implementation-planner の前提になるため先行確定が必要
- D-1〜D-8 は先例準拠の設計者裁量として記録したが、本書全体が draft であり確定判断ではない。
  異論があれば S-x と同時に指摘を受けて修正する
