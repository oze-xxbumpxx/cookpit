...
| R-3 | 4 集約更新が非トランザクション（部分失敗の窓自体は残る）                                                                                         | **解消済み（ADR-0019）**。書き込み UseCase を `UnitOfWork.execute` で 1 トランザクションに閉じる。冪等再実行は防衛線として残す                                                                                              |
...
- 横断: DB トランザクション導入（R-3）→ ADR-0019 で解消。冪等再実行は防衛線として残す
  </content>
