...
| R-8 | **`amount_value` が `numeric(10, 3)` のため、R-6 の「0 の在庫を作らない」3 段防御
（Zod `positive()` / Domain `<= 0` / 概念上の禁止）を `0.0001` がすり抜ける** | `0.0001` は
契約層（`positive()`）も Domain（`<= 0`）も通過するが、DB では `0.000` に丸められる。
リロードすると「数量 0 の在庫」が一覧に残り、`isEmpty()` が真になる在庫が集約から除去されずに
存在する状態になる | 本ユニットでは**受容する**（発生させるには意図的に 0.001 未満を入力する
