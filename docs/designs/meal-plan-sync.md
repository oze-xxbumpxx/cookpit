...
- 関連:
  ...
  - [ADR-0007](../decisions/ADR-0007-shopping-list-differential-merge.md)（差分マージ・明示トリガの
    決定。決定2「数量合算しない」・決定6「削除追随は将来課題」を本書が塗り替える）
...
- **(c) 冪等性**: 同一の献立に対して連続して同期を実行すると、2 回目以降は追加・更新・削除の
  いずれも発生せず no-op になる（FR-7）。ただし Pantry 在庫が部分的に消費された状態での再同期は
  厳密な no-op にならない可能性がある（§リスク R-4）。専用の冪等性キーは導入しない
...
| R-3 | ShoppingList の保存が成功し Pantry の保存が失敗する部分失敗窓                                                                                                                                                                | Pantry の在庫消費が買い物リストの数量変更と食い違う（在庫が減っていないのに数量が減って見える等）                    | 本ユニットでは対応しない。Sprint 10 Unit B（DB トランザクション / UoW 導入）で閉じる（§対象外・§エラー処理(d)）                                                                                                                                                                                                       |
| R-4 | Pantry 在庫が部分的に既に消費された品目に対し、同一の献立を変更なく再同期すると、delta 計算（新しい集計の生値 − 現在の品目値〈買う量〉）が正の値になり、既に消費済みの在庫分をもう一度「増分」として扱ってしまう可能性がある | 厳密な意味での冪等性（FR-7/N-9）が Pantry 非空の状況では保証されない。品目の `requiredAmount` が意図せず増える可能性 | 受容する（本ユニットでは対応しない）。Pantry がほぼ空の運用（在庫を作り置き前提で早めに消費する Cookpit の想定利用）では発生しにくい。試験計画では Pantry を空にしたシナリオで冪等性を確認し、Pantry 非空時の挙動は既知の限定として記録する。将来、必要になれば「集計時の生値を品目側に保持する」等の拡張で解消できる |
