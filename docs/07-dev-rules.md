...
- **本番の Drizzle 接続は `neon-http`（HTTPS）**。Sprint 10 Unit B で `neon-serverless`
  の WebSocket `Pool` に切り替えたところ、`GET /api/health` が `db: "error"`、画面は
  `loading.tsx` のあと RSC digest で落ちた（2026-08-13 実測。ADR-0019 ロールバック実行記録）。
  **失敗の原因は切り分けできていない**。`ws` のバンドル・`globalThis` にキャッシュした `Pool` の
  陳腐化・WebSocket 非到達の 3 候補があり、どれかは未確定（ADR-0019 §原因の切り分けは未了）。
  対話型トランザクションの再導入は、候補を切り分けたうえで Preview の書き込み一巡を通してから。
- **本番の接続方式を変える変更は、自動試験では検証できない**。テストは PGlite だけで、
  Neon への接続方式・保存確定・巻き戻しを観測しない。Preview で書き込みを一巡させる工程を
  持たない限り、`pnpm test` が緑でも本番が落ちうる（出典: PR #164 → #168 の障害）。
