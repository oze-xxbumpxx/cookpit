...
- 位置づけ: **MVP2 の最終スプリント**。本レビューで Sprint 8 のクローズ記録（2026-08-15 付）を
  追加したため、MVP2 スケジュール表の 4 本すべてがクローズ済みになった（後述の roadmap 更新提案 1）
...
1. **【適用済み・2026-08-17 ユーザー確定】MVP2 スケジュール表の Sprint 8 行**に完了マークが
   無かった。完了条件 3 件はすべて `[x]` で、3 件目は 2026-08-15 に達成済み（Sprint 10 タスク 5
   として消化）。**クローズ日を 2026-08-15（完了条件 3 件目の達成日）**として、表の
   「✅ **クローズ済み**（2026-08-15）」表記と「Sprint 8 クローズ」節を `docs/05-roadmap.md` へ追加した
2. **【適用済み・2026-08-17 ユーザー確定】MVP2 全体の締め節が無かった。** 4 スプリントすべてが
   クローズ済みになったので、「MVP2 クローズ」節（完了条件 10 件と証跡の対応表・ゴールの優先軸
   「実運用の穴 > 新機能 > 基盤」との対応・Phase 3 への申し送り）を `docs/05-roadmap.md` の
   「MVP2 に含めないもの（Phase 3 候補）」の直前へ追加した（PR #177）。置き場所は roadmap が
   適切と判断（各スプリントのクローズ節と同じ文書に並べる）。あわせて MVP2 の「ステータス」節を
   「骨子（未確定）」からクローズ済みへ更新した。**2 人での実運用 1 周の観察は未実施**である点を、
   Sprint 6 と同じ反証不能クラスとしてブロッカーにしない形で明示している
