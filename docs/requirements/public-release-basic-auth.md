...
- F-05c `401` / `503` はいずれも **`Content-Type: text/html; charset=utf-8` と描画可能な本文**を
  返す。本文も Content-Type も無い応答はブラウザがレンダリングできず、不明なファイルとみなして
  ダウンロードを提案する。401 は実体が無いためダウンロードも完了せず、利用者は復帰手段を失う
  （2026-09-16 に iOS Safari で顕在化）。本文にアプリの情報・資格情報・環境変数名は含めない。
- F-07 Service Worker は **401 を `respondWith` しない**。SW が返した 401 に対してブラウザは
  Basic 認証ダイアログを出さないため（Chromium issue 623464）、画面遷移で 401 を受けた場合は
  SW 非経由の `/` へ 302 で誘導し、ブラウザに認証を引き受けさせる。401 はキャッシュしない。
