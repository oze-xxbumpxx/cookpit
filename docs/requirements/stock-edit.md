...
- 作成日: 2026-08-09（2026-08-09 更新: P-1〜P-6 ユーザー確定を反映）
...
- 在庫（Stock）の賞味期限・保存場所・数量を事後に編集できるようにする（roadmap タスク 1。
  数量を含めることは 2026-08-09 のユーザー確定 P-1 による）。
...
- ユーザー確定（2026-08-09、`docs/05-roadmap.md` L652-665）: タスク 1・2 は 1 ユニット
...
- ユーザー確定（2026-08-09、P-1〜P-6）: 設計書 `docs/designs/stock-edit.md` §確定事項を参照。
...
- E-6: 買い物完了パネルで不正な日付形式の賞味期限が入力された場合の扱い（クライアント側で
  事前に弾くか、API のエラーを品目単位でハンドリングするか）は設計側 P-3 の UI 設計に含める。
...
- Domain 層の不変性方針の変更（`pantry-core.md` S-4 案 α からの転換）を伴うため、
  [ADR-0016](../decisions/ADR-0016-stock-details-mutable.md) に記録する（作成済み・2026-08-09）。
...
- [x] ADR-0016 の起票（設計フェーズで完了・2026-08-09）。

## 未決事項

なし（P-1〜P-6 は 2026-08-09 にユーザー確定済み。確定内容と比較検討の記録は
