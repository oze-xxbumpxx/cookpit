...
- キュー対象はチェック操作（`POST .../items/:itemId/checked`）のみ。`bought` / `DELETE` /
  `consume` は対象外。
- Background Sync API（SyncManager）は不採用。理由は Chromium 系限定で iOS Safari が未対応と
  見られるため（Cookpit の主対象は iPhone の PWA）。ただしこの対応状況は**一次情報での裏取りが
  できていない**（本タスク実行環境は developer.mozilla.org / caniuse.com への egress が遮断
  されている）。
...
- Background Sync API（`ServiceWorkerRegistration.sync` / `SyncManager`）は採用しない。
  **未確認の前提**: iOS Safari が Background Sync 未対応という判断は、本タスク実行環境が
  developer.mozilla.org / caniuse.com への egress を遮断されているため一次情報で検証できて
  いない。もしこの前提が誤りで iOS Safari が対応していることが判明した場合、`sw.ts` の
  `addEventListener('sync', ...)` + `ServiceWorkerRegistration.sync.register()` によるブラウザ
  ネイティブの再送機構（バックグラウンドでタブを閉じていても再送される）を追加検討できるが、
  これは `sw.ts` への変更を伴うため本要件のスコープ外（案C固定）のまま変わらない。実装着手前に
  Web 検索が可能な環境で一次情報を確認することを推奨する（§未決事項）。
...
- Background Sync API（`SyncManager`）の採用（§前提）。将来 iOS Safari の対応状況が確認でき、
  かつ `sw.ts` への変更が許容される段階で再検討する候補として申し送る。
...
1. **Background Sync 未対応（iOS Safari）の一次情報未確認**（§前提）。実装着手前に Web 検索可能な
   環境で developer.mozilla.org / caniuse.com 等を確認することを推奨する。Orchestrator 経由で
   確認要否をユーザーに諮る。
