...
- B-6: Pantry 在庫が完全に空でない状態（同期時点で在庫が既に部分消費されている）で同一献立を
  2 回同期すると、2 回目が厳密な no-op にならない可能性がある（設計書 §リスク R-4 で詳細化。
  既知の限定として受容し、本ユニットでは対応しない）。

## 前提

- ADR-0007 の決定 1〜3・6（差分マージ・明示トリガ・マッチキー）は維持する。本ユニットは
  ADR-0007 の決定 2（数量合算しない）・決定 6（削除追随は将来課題）を Gate A の確定内容で
  塗り替える。
