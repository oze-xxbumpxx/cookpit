...
- **第三者が自分の購読を登録して在庫名ダイジェストを受信できる経路が残る。**
  `GET /api/push/vapid-public-key` が公開鍵を無認証で配るため、アプリ URL を知る者は
  「公開鍵を取得 → 自分のブラウザで購読を作る → `POST /subscribe`」で登録できる。
  **`endpoint` の推測困難性はこの経路を防がない**（推測困難性が効くのは他人の購読の削除と
  なりすまし送信に対してである）。security-reviewer が設計フェーズで検出した（H-1）。
  購読件数の上限（10 件。既存 `endpoint` の再登録は対象外）で影響を限定し、上限内で
  1 件受信される可能性は単一世帯前提のもとで受容する（設計書 P-15）。ADR-0003 を覆さない
  範囲での対処であり、認証の導入は Phase 3 の課題のまま。
- **`endpoint` は攻撃者が制御しうる外部 URL であり、Cron がそこへ毎日 POST する**（blind SSRF）。
  IP リテラルと内部ホスト名を拒否することで軽減するが（設計書 P-16）、正規の Push Service に
  見えるホストへの送信は防げない。Vercel の実行環境から内部アドレスへ到達できるかは未確認。
...
- 設計書: `docs/designs/expiry-alert.md`（ステータス confirmed。P-1〜P-17 の確定内容と
  実装上の罠 9 件）
