...
- 実装状況: **実装済み**（2026-08-04。レビュー対応まで完了）。試験計画作成時点は未実装だったため以下は当時の記述を残す。当初は `_utils/price-comparison.ts` / `_components/store-unit-price-list.tsx` は存在しない。以降の観点は設計書どおりの実装を前提にした計画であり、実装後にファイルパス・シグネチャの乖離がないか照合すること。
...
| PC-20b | 縮退#7 の全件版: **全記録の `storeName` が空文字** → 例外を投げず双方 `null`（レビュー M-1 の回帰） | `product.priceHistory` の 2 件すべてが `storeName === ''`                                                                                 | `estimateItemPriceDiff` / `buildStoreUnitPriceBreakdown`                             | どちらも `null` を返し `TypeError: Reduce of empty array with no initial value` を投げない                                    | 異常（データ不整合の防御） |
| PC-20c | 縮退#7 の境界: 空文字を除くと**有効な記録が 1 店舗だけ**残る → 内訳は `null`                        | 有効 1 件 + 空文字 1 件                                                                                                                   | `buildStoreUnitPriceBreakdown`                                                       | `null`（2 件未満）                                                                                                            | 境界                       |
...
| PC-32 | 基準区分は**最頻の kind**（少数派の raw 最小単価に引きずられない。レビュー S-3 の回帰） | `other` 2 件（`1L` で 200 / 150）+ `volume` 1 件（`ml` で 18）。raw 最小は volume 側 | `buildStoreUnitPriceBreakdown` | `other` の 2 件が内訳に残る（`basisLabel` は `11L`。既知の限定事項 2 を同時に固定する） | 正常（アルゴリズム） |
| PC-33 | 基準区分が同数のときは `weight` → `volume` → `other` の固定順（tie-break の決定性） | `volume` 2 件 + `weight` 2 件 | `buildStoreUnitPriceBreakdown` の `basisLabel` | `'100g'`（weight が優先される） | 境界 |
| PC-34 | 非最安でも丸め後の差が 0 なら「ほぼ同額」（レビュー S-1） | 同一 kind の 2 件（`unitPriceAmount` が `100` と `100.4`） | `buildStoreUnitPriceBreakdown` + `formatStoreUnitPriceDiffLabel` | 2 行目は `isCheapest: false` / `diffFromCheapestYen: 0` で、ラベルは `'ほぼ同額'`（`'+0円'` ではない） | 境界 |
| PC-35 | `formatStoreUnitPriceDiffLabel` は差がある行を `+N円` で返す | `isCheapest: false` / `diffFromCheapestYen: 30` | `formatStoreUnitPriceDiffLabel` | `'+30円'` | 正常 |
...
| --------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `_utils/price-comparison.ts`            | `estimateItemPriceDiff`                                              | PC-01〜PC-09, PC-13, PC-15〜PC-21, PC-20b, PC-29〜PC-31                                                                    |
| `_utils/price-comparison.ts`            | `buildStoreUnitPriceBreakdown`                                       | PC-14〜PC-18, PC-20, PC-20b, PC-20c, PC-22〜PC-30, PC-32〜PC-34                                                            |
| `_utils/price-comparison.ts`            | `formatEstimatedDiffMessage`                                         | PC-12                                                                                                                      |
| `_utils/price-comparison.ts`            | `formatYen`                                                          | PC-10, PC-11                                                                                                               |
| `_utils/price-comparison.ts`            | `formatStoreUnitPriceDiffLabel`                                      | PC-34, PC-35                                                                                                               |
...
- §2（PC-01〜PC-31, PC-20b, PC-20c, PC-32〜PC-35, SUP-01〜07）・§3（IR-25〜37, PF-09〜11, SG-08〜09, LC-29〜34）の全観点が実装され green。
  （PC-20b/20c・PC-32〜35・LC-33/34 は 2026-08-04 のレビュー対応で追加・実装済み。`apps/web` の test は 700 → 708 件）
...
- §7 の実画面確認 MB-01〜06 が実装後に実施され PASS している。
  → **2026-08-04 実施・全 PASS**。実施記録と所見（MB-01 の 2 行折り返し）は
  `docs/reviews/shopping-list-price-comparison.md` §S-7 を参照。
