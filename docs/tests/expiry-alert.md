...
> `BLOCKED`（実機なし）。MB-05〜07・MB-11〜14 は未実施。判定の全量は §10-5、
> 実施の経緯は `logs/2026-08-15.md`。
...
**実施者**: ユーザー本人。**端末**: iPhone（Safari / ホーム画面追加済み PWA = PS-2）。
**環境**: Preview ではなく**本番**（Preview の Deployment Protection が iPhone PWA での
確認を阻むため）。**Android 実機は用意できなかった。**

判定の出典は `logs/2026-08-15.md` の実施記録（本番 Cron 手動実行の戻り値まで残っている）。
§10 冒頭の「該当環境が無いため確認できず」を PASS として報告しない運用に従い、
**実際に観測されたもの以外はすべて未実施**として残す。

| ID     | 判定       | 備考                                                                               |
| ------ | ---------- | ---------------------------------------------------------------------------------- |
| MB-01  | **未実施** | Android 端末なし（PS-1 未シード）                                                  |
| MB-02  | **未実施** | Android 端末なし。iOS 側の OFF 経路は MB-xx に対応項目が無い                       |
| MB-03  | PASS       | iPhone ホーム画面 PWA で購読成功（`subscriptionCount: 1`）                         |
| MB-03b | **未実施** | ホーム画面未追加タブ（PS-3）での対照を取っていない                                 |
| MB-04  | PASS       | 本番 `GET /api/push/vapid-public-key` が 200                                       |
| MB-05  | **未実施** | ES-1〜ES-6 のシードを行っていない。通知本文の要約形式は未確認                      |
| MB-06  | **未実施** | 同上（ES-2 未シード）                                                              |
| MB-07  | **未実施** | 同上（ES-5 未シード）。**P-10b の非対称は実機で未確認**                            |
| MB-08  | PASS       | 本番 Cron 手動実行後に iPhone へ通知が届いた（`sentCount: 1`）。**完了条件の実体** |
| MB-08b | **未実施** | Android 対照が取れていない（罠 8・R-9 の検出経路が空いたまま）                     |
| MB-09  | **未実施** | 通知タップからの `/pantry` 遷移は記録に無い                                        |
| MB-09b | **未実施** | Android 端末なし                                                                   |
| MB-10  | PASS       | ヘッダーなし → 401 / 誤値 → 401 / 正値 → 200                                       |
| MB-11  | **未実施** | 在庫 0 件状態での無通知は未確認（N-7）                                             |
| MB-12  | **未実施** | 購読デバイスが 1 台のみ（N-8）                                                     |
| MB-13  | **未実施** | 375px でのレイアウト目視を行っていない                                             |
| MB-14  | **未実施** | **O-01 の回帰確認（REG-07）が未実施**                                              |

**合計: PASS 4 件 / 未実施 13 件。**

**シードデータの消化状況**（§10-3 手順 12）: **ES-1〜ES-6 は未シード**、PS-1・PS-3 も未使用。
使ったのは PS-2（iPhone PWA）のみ。§10-3 手順 12 の「前提データ全消化チェック」は
**満たしていない**。

> **判定を訂正した経緯（2026-08-15）**: 本節は当初「iOS 側 12 項目 PASS」と記録したが、
> 実施を伴走したセッションの記録（`logs/2026-08-15.md`・PR #170）と突き合わせたところ、
> 実際に観測されたのは MB-03 / 04 / 08 / 10 の 4 件だけだった。**口頭の「全項目 PASS」を
> そのまま転記したのが誤りの原因**で、実施記録を一次情報として上書きした。

**残る穴 — 追跡しない（2026-08-15 ユーザー判断）**:

以下は未実施のまま残るが、**追加確認は行わないとユーザーが判断した**。持ち越しタスクとして
起票せず、本節を最終記録とする。将来 Android 対応や複数デバイス利用が要件に入った時点で
再訪する。

1. **Android 対照（MB-08b）が取れていない。** 試験計画が「必須」に指定している項目で、
   `VAPID_SUBJECT` の形式ミスなど **iOS だけ落ちる/Android だけ落ちる**種類の障害
   （罠 8・R-9）は、片側だけでは検出できない。Cookpit の主対象は iPhone PWA で、
   Android は現在の利用形態に含まれないため追わない。
2. **通知の中身が一度も検証されていない（MB-05 / 06 / 07）。** ES-1〜ES-6 を
   シードしていないため、「先頭 3 件 + 他 n 件」の要約形式も、期限切れの取り込み（B-2）も、
   **P-10b の非対称（数量実質 0 は画面に出て通知に出ない）も実機では未確認**。
   確認できたのは「通知が 1 通届く」ところまで。設計上の分岐は自動テスト
   （SEA-04/06/07・GES-03）が押さえている。
3. **MB-14（REG-07）の O-01 回帰が未実施。** `sw.ts` に `push` / `notificationclick` を
   足した後、オフライン再訪問が壊れていないことを実機で見ていない。自動テストが存在しない
   領域（§0-2）なので、**ここだけは検知手段が実質ゼロ**である点は記録に残す。
4. **複数デバイス配信（MB-12）・通知タップ遷移（MB-09）・375px レイアウト（MB-13）・
   在庫 0 件の無通知（MB-11）・ホーム画面未追加タブの対照（MB-03b）も未実施。**
5. **iOS 側の購読 OFF（解除）に対応する MB 項目が無い。** MB-02 が Android 前提で書かれて
   いるため、iOS で OFF を押した後に通知が止まることは今回の範囲外。自動テスト側は
   `UnsubscribeFromExpiryAlertUseCase`（UNSUB-01〜02）と `pushRoute`（WH-PUSH-09/10）で
   解除経路を押さえている。
6. **§10-3 手順 13（`subscriptionCount` の想定台数確認）は実質確認済み。**
   本番 Cron の戻り値が `subscriptionCount: 1` で、検証端末 1 台と一致している。
   M-3（Preview 用鍵の破棄）は**該当しない** — 今回は Preview を使わず本番で確認したため、
   Preview 専用鍵を発行していない。
...
- [~] manual-browser-verify（§10）: MB-01〜MB-14 をすべて実施し、ES-1〜ES-6・PS-1〜PS-3 の
  ...
  （M-3）。
  → **2026-08-15 に部分実施（§10-5）。この状態で打ち切る（ユーザー判断）。**
  **PASS は MB-03 / 04 / 08 / 10 の 4 件のみで、残り 13 件は未実施**。必須指定の
  MB-07（P-10b の非対称）・MB-08b（Android 対照）・MB-14（O-01 再確認）がいずれも
  欠けており、ES-1〜ES-6 も未シードなので、本条件は**大きく未充足のまま閉じる**。
  手順 13 は本番 Cron の `subscriptionCount: 1` で実質確認済み、M-3 は Preview を
  使っていないため該当なし。**未実施分の追加確認は行わない**と 2026-08-15 に
  ユーザーが決めたため、本項目はこのまま閉じる（§10-5「残る穴」）。
- [x] roadmap Sprint 8 完了条件 3 件目「期限が近い在庫にアプリを開かずに気づける」を
      満たすことを実機確認後に確認する。
      → **達成（2026-08-15）**。利用者の実機 iPhone（PWA）で本番 Cron の手動実行後に
      通知を受信した（MB-08。`sentCount: 1`）。Cookpit の主対象端末で「アプリを開かずに
      気づける」経路が通ったため条件は満たすと判断する。**通知の中身（MB-05〜07）と
      タップ遷移（MB-09）は未確認**である点は §10-5「残る穴」に残す。
