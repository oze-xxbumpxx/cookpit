...
- 作成日: 2026-08-09
- 前提となる設計書: `docs/designs/expiry-alert.md`（P-1〜P-14 ユーザー確定・2026-08-09・confirmed）
...
- 更新: 2026-08-09（提出後にユーザー確定した P-13/P-14 を反映）。
  ...
  **全 6 export**を `packages/application/src/pantry/expiry.ts` へ移設し、
  ...
  据え置き・変更なし）。この 2 点に伴い §0-1・§0-2・§1・§3・§5-3・§7-2・§7-3・§8・§13・§15 を
  修正した（修正箇所は本文中に注記）。設計書・契約設計書はこちらで更新済みのため本書は
  再読していない（新しい行番号は引用しない）。
...
| **Domain（P-13 確定・配置変更）**      | `PushSender`（interface。`PushPayload`/`PushSendResult`/`PushSubscriptionTarget` を伴う）                                                                      | `send(subscription, payload): Promise<PushSendResult>`                                                                                                                                                                          | 設計書 §確定事項 P-13（`packages/domain/src/push-subscription/push-sender.ts`。**当初は Application 層のポートとして設計されていたが、`packages/infrastructure` が `@cookpit/domain` にのみ依存するため Domain 層へ移設**。行番号は更新後の設計書を本書では再読していないため付与しない） |
...
| Infrastructure                         | `WebPushSender`（**P-13 確定**: `PushSender`（`@cookpit/domain`）を実装する）                                                                                  | `send(subscription, payload): Promise<PushSendResult>`                                                                                                                                                                          | 設計書 L551-576（`implements PushSender` の import 元が `@cookpit/domain` に変更）                                                                                                                                                                                                        |
...
| **apps/web UI（P-14 反映・分裂解消）** | `dashboard.tsx`（既存・改修）                                                                                                                                  | `getExpiryRemainingDays`/`getExpiryUrgency`/`formatExpiryUrgencyLabel` の**3 関数すべて**を `@cookpit/application` から import する（`expiryUrgencyChipClass` のみ据え置きで `@/app/_utils/category-color` から import）        | 実測（移設前）`apps/web/src/app/_components/dashboard.tsx` L4-8。**当初は `formatExpiryUrgencyLabel` だけ apps/web に残り import 元が分裂する想定だったが、P-14 の全移設確定によりこの分裂は解消し、3 関数とも同じ import 元になる**                                                      |
| **apps/web UI（P-14 反映・分裂解消）** | `stock-row.tsx`（既存・改修）                                                                                                                                  | 同上（3 関数すべて `@cookpit/application` から import）                                                                                                                                                                         | 実測（移設前）`apps/web/src/app/pantry/_components/stock-row.tsx` L1-8。同上の理由で分裂は解消                                                                                                                                                                                            |
| apps/web                               | `page.tsx`（ダッシュボード。既存・改修）                                                                                                                       | `selectExpiringStocks` 直接呼び出しを `GetExpiringStocksUseCase` に差し替え                                                                                                                                                     | 実測 `apps/web/src/app/page.tsx`（全 19 行）。現状は `import { selectExpiringStocks } from './_utils/dashboard-view'` + `import { EXPIRY_URGENCY_WITHIN_DAYS } from './_utils/expiry'` を直接使用（`expiry.ts` 削除後は `@cookpit/application` からの import に統一）                     |
...
- **`dashboard.tsx` は `'use client'` を持たない**（実測: L1 から import 文で始まり
  ...
  **設計書 R-12（バンドルサイズ増の懸念）は `dashboard.tsx` には該当しない**（P-14 により
  `formatExpiryUrgencyLabel` も含めた 3 関数すべてが `@cookpit/application` からの import に
  なるが、Server Component であるため結論は変わらない）。
- **`stock-row.tsx` も自身は `'use client'` を持たない**（実測: L1-14 に該当ディレクティブ無し）。
  ...
  （P-14 により `formatExpiryUrgencyLabel` も同じ import 元に加わるため、バンドルに含まれる
  関数が 2 個から 3 個に増える点も確認対象に含める）。
  試験計画としては**アサーション困難な観点**のため自動テストの対象にはせず、実装計画の
  申し送りとして§15 に記録する。
- **既存 `apps/web/tests/app/_utils/dashboard-view.node.test.ts`（実測・全 196 行）は
  P-4/P-14 の移設対象関数のテストを大量に含んでいる。** 具体的には:
  ...
  - **（P-14 反映・更新）** `describe('formatExpiryUrgencyLabel', ...)`（L156-195、
    EU-13〜EU-18）: 当初は「`formatExpiryUrgencyLabel` は apps/web に残る」前提で
    このテストブロック自体は残す想定だったが、**P-14 確定により `formatExpiryUrgencyLabel` も
    Application 層へ移設される**ため、このテストブロックも `selectExpiringStocks` 等と
    同じく移設対象になる。EU-18（L177-194、`getExpiryUrgency` と `formatExpiryUrgencyLabel`
    の連動を確認する複合テスト）は両関数が同一パッケージに揃うため、**import 元の分裂を
    気にする必要がなくなり移設がシンプルになる**（当初懸念していた「cross-layer な
    import が必要になる」問題は解消）。
  - **この既存ファイルの扱いは設計書・要件書のどちらにも明記が無かったが、本書提出後に
    P-14 の確定によって「`MEAL_PLAN_STATUS_LABELS` 以外は全て移設」という単純な結論に
    収束した。** 元ファイルの配置は変更不要（後述 REG-06・§15 項目 1 参照）。§7-5・§8 で
    移設先の詳細を示す。
...
| Domain（**P-13 確定**）            | `PushSender`（interface）                                                              | `send(subscription, payload)`                                                                                                                                                       | 新設・配置変更   | 専用のテスト対象は無い（interface）。実装側 `WebPushSender`（INF-WPS-01〜06）とモック利用側 `SendExpiryAlertsUseCase`（SEA-\*）でカバー |
...
| Application（**P-14 確定・拡張**） | `expiry.ts`（移設）                                                                    | `parseExpiryDate`/`toLocalMidnight`/`getExpiryRemainingDays`/`getExpiryUrgency`/`formatExpiryUrgencyLabel`/`selectExpiringStocks`/`EXPIRY_URGENCY_WITHIN_DAYS`（**6 export 全て**） | 移設（挙動不変） | APX-01〜24（既存 EU-01〜18・EXP-03 の移設。`formatExpiryUrgencyLabel` 分の APX-19〜24 を追加）                                          |
...
| Infrastructure                     | `WebPushSender`（`PushSender` を実装。**P-13 確定で `@cookpit/domain` の interface**） | `send(subscription, payload)`                                                                                                                                                       | 新設             | INF-WPS-01〜06                                                                                                                          |
...
| apps/web UI（**P-14 反映**）       | `dashboard.tsx`                                                                        | import 元差し替え（3 関数とも `@cookpit/application`）                                                                                                                              | 変更             | DASH-REG-01〜02                                                                                                                         |
| apps/web UI（**P-14 反映**）       | `stock-row.tsx`                                                                        | import 元差し替え（3 関数とも `@cookpit/application`）                                                                                                                              | 変更             | SR-REG-01〜02                                                                                                                           |
...
| PS-08（防御性・要確認） | `createdAt` getter の防御的コピー    | `reconstruct()` で復元した `PushSubscription`                                  | `const d = subscription.createdAt; d.setFullYear(1900);`     | 再取得した `subscription.createdAt` が変化しない（**Unit A の `Stock.purchasedAt`/`expiresAt` と同じ防御的コピーパターンを踏襲する前提。設計書 §将来課題に「実装時に確定」と明記された。§15 項目 4 参照。要確認のまま残す**） | 防御性・要確認           |

`PushSender`（P-13 確定で Domain 層に配置。§0-1・§1 参照）は interface のみのため、
本節では独立したテストケースを設けない。挙動の確認は §5-3（実装 `WebPushSender`）と
§3-3（利用側 `SendExpiryAlertsUseCase`）で行う。
...
`@cookpit/domain` からの型（P-13 確定）**。呼び出しごとの結果をテストケース側から差し込める
フェイク）を新設する。
...
| SEA-10（P-7・**P-14 反映**）     | 通知本文の組み立て                                      | 期限が近い在庫 5 件（閾値内）                                                                             | `execute(asOf)`                                                      | `pushSender.send()` に渡る `payload.body` が、`formatExpiryUrgencyLabel`（`@cookpit/application`。P-14 確定により両関数が同一パッケージにあるため直接利用できる）を用いて組み立てられた先頭 3 件の文言 +「他 2 件」の形式。`payload.url === '/pantry'` | 正常（N-3, P-7）          |
...
| APX-19〜24（**P-14 で追加**） | `formatExpiryUrgencyLabel`（EU-13〜EU-18。期限切れ・本日まで・明日まで・あと N 日・`getExpiryUrgency` との連動確認）                    | 既存 L156-195（6 ケース。EU-18 は `getExpiryUrgency`+`formatExpiryUrgencyLabel` の複合テストだが、両関数が同一パッケージ内の同一テストファイルに揃うため import 元を分ける必要がなくなり、そのまま移設できる） | 正常・境界（回帰）   |
...
経由）に変わる。**P-14 確定により `formatExpiryUrgencyLabel` も含めた 6 export すべてが
同一の移設先（`packages/application/src/pantry/expiry.ts`）に揃うため、当初懸念していた
「一部だけ apps/web に残し import 元が割れる」複雑さは発生しない。**
...
§10.1 の観点表に ID を付与し、追加ケースを補う。P-13/P-14 は api-contract の型契約
（Zod スキーマ・レスポンス形）には影響しないため、本節に変更は無い。
...
| EAS-09（要確認・§15） | 初期表示時の購読状態判定方法                                           | 既に購読済みのブラウザで再訪問                                                             | render                                                                                                                                                                                                                     | ON/OFF どちらの表示になるか（`pushManager.getSubscription()` を mount 時に呼ぶ設計になっているかは設計書 §将来課題に「実装時に確定」と明記された。§15 項目 3 参照。要確認のまま残す） | 要確認                          |
...
| #           | 観点                                 | 前提                                                                                                                                                                                                                                                                                                                                                               | 操作                        | 期待結果                                                                 | 分類        |
| ----------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------- | ------------------------------------------------------------------------ | ----------- |
| DASH-REG-01 | 緊急度チップ・ラベルの表示が挙動不変 | 実測（移設前）L1-8 の import 元を、`getExpiryRemainingDays`/`getExpiryUrgency`/`formatExpiryUrgencyLabel` の**3 関数すべて** `@cookpit/application` から import する形に差し替えた後の既存テスト一式を実行（P-14 確定により「`formatExpiryUrgencyLabel` だけ apps/web に残す」分裂は発生しない。`expiryUrgencyChipClass` は `@/app/_utils/category-color` のまま） | `pnpm test`（apps/web dom） | 既存の全アサーションが変更前と同じ結果で pass する                       | 回帰        |
...
| #         | 観点                                                                                                                                                                                            | 前提                                      | 操作                             | 期待結果                                                                | 分類 |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------------------------------- | ----------------------------------------------------------------------- | ---- |
| SR-REG-01 | 緊急度チップの閾値内外出し分けが挙動不変                                                                                                                                                        | 実測（移設前）L1-8 の import 元差し替え後 | 既存の閾値内外テストケースを実行 | 全件 pass（Unit A の `SR-EDIT-03/04` 相当の既存観点が壊れていないこと） | 回帰 |
| SR-REG-02 | `formatExpiryUrgencyLabel` を含む 3 関数すべてが同じ `@cookpit/application` の import 元で動作する（当初の「据え置きの import 元でも動作する」という前提は、P-14 の全移設確定により解消された） | 同上                                      | render                           | ラベル文言（「期限切れ」「本日まで」等）が変更前と一致                  | 回帰 |
...
| REG-03                                    | 既存 Application（`AddStockUseCase`/`ConsumeStockUseCase`/`DiscardStockUseCase`/`GetPantryUseCase`/`UpdateStockDetailsUseCase`） | `pnpm test`（packages/application）全件 pass                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 新規 UseCase・`expiry.ts` 移設（P-14 で全 6 export に拡大）が既存 UseCase に影響しないこと                                              |
...
| REG-06（**最重要・P-14 で結論が単純化**） | **`apps/web/tests/app/_utils/dashboard-view.node.test.ts` の再構成**（§0-2・§3-5）                                               | 移設後、`MEAL_PLAN_STATUS_LABELS` の `describe`（実測 L80-88）**のみ**を含む状態で全件 pass する。`selectExpiringStocks`/`getExpiryRemainingDays`/`getExpiryUrgency`/`EXPIRY_URGENCY_WITHIN_DAYS`/`formatExpiryUrgencyLabel`（P-14 で追加）のテストは `packages/application/tests/pantry/expiry.test.ts`（APX-01〜24）で全件 pass する。**ファイル名・配置は変更不要**: 移設後も `apps/web/src/app/_utils/dashboard-view.ts` は `MEAL_PLAN_STATUS_LABELS` の定義を保持し続けるため（`selectExpiringStocks` のみ削除される）、テストファイル名 `dashboard-view.node.test.ts` は src の構造（`tests/` が `src/` をミラーする規約）と引き続き整合する | P-4/P-14 の移設が既存テスト資産を失わせないこと。設計書・要件書に明記が無かったが、P-14 確定により結論が明確になった（§15 項目 1 参照） |
...
| REG-10                                    | 型・Lint                                                                                                                         | `pnpm type-check` / `pnpm lint`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `AppType` 拡張・`api-contract` の `export *` 追加、および `PushSender` の配置変更（P-13）に影響がないこと                               |
...
- [ ] Application: SUB-01〜05・UNSUB-01〜02・SEA-01〜11・GES-01〜05・APX-01〜24 が実装され
      pass する（**SEA-04/06/07 と GES-03 の非対称固定は必須**）。`PushSender` 関連の型は
      `@cookpit/domain` から import されていることを確認する（P-13）。
...
- [ ] 回帰試験範囲（§8 REG-01〜10）がすべて pass する。**特に REG-06（既存
      `dashboard-view.node.test.ts` の再構成。P-14 確定により `MEAL_PLAN_STATUS_LABELS`
      のみが残る）と REG-07（O-01 の手動再確認）は明示的に実行結果を確認する。**
...
| `packages/application`                                                                    | `tests/**/*.test.ts`                                  | `packages/application/tests/notification/subscribe-to-expiry-alert.use-case.test.ts`（新規）、`unsubscribe-from-expiry-alert.use-case.test.ts`（新規）、`send-expiry-alerts.use-case.test.ts`（新規）、`packages/application/tests/pantry/get-expiring-stocks.use-case.test.ts`（新規）、`packages/application/tests/pantry/expiry.test.ts`（新規・既存 apps/web テストの移設先。P-14 で全 6 export 分） |
...
| `apps/web`（node。実測 include: `tests/**/*.node.test.ts` / `tests/server/**/*.test.ts`） | 同左                                                  | `apps/web/tests/server/routes/push.test.ts`（新規）、`apps/web/tests/server/routes/cron.test.ts`（新規）、`apps/web/tests/app/_utils/dashboard-view.node.test.ts`（再構成。`MEAL_PLAN_STATUS_LABELS` のみ残す。§3-5・§8・§15 項目 1）                                                                                                                                                                    |
...
1. **【解決済み・2026-08-09 P-14 確定】既存 `apps/web/tests/app/_utils/dashboard-view.node.test.ts`
   の扱いが、提出時点では設計書・要件書のどちらにも明記が無かった。** P-4 の移設対象
   （`selectExpiringStocks`/`getExpiryRemainingDays`/`getExpiryUrgency`/
   `EXPIRY_URGENCY_WITHIN_DAYS`）のテストがこのファイルに集中しており（実測 L26-153）、
   加えて `formatExpiryUrgencyLabel`（L156-195）の扱いが「apps/web に残る」前提と
   「通知本文がこの関数に依存する」（P-7）という前提が両立しないという矛盾を提出時に
   指摘していた。**P-14 確定（`formatExpiryUrgencyLabel` を含む全 6 export を Application 層へ
   移設し `apps/web/src/app/_utils/expiry.ts` を削除）によりこの矛盾は解消済み。**
   結論: `packages/application/tests/pantry/expiry.test.ts`（新規）へ 6 export 分すべて
   （APX-01〜24）を移設し、元ファイルには期限判定と無関係な `MEAL_PLAN_STATUS_LABELS` の
   `describe`（L80-88）のみが残る。**ファイル名・配置の変更は不要と判断する**:
   `apps/web/src/app/_utils/dashboard-view.ts` 自体は `MEAL_PLAN_STATUS_LABELS` の定義を
   持ち続ける（`selectExpiringStocks` だけが削除される）ため、テストファイル名
   `dashboard-view.node.test.ts` は「`tests/` が `src/` の構造をミラーする」既存規約
   （`.claude/rules/coding-standards.md`）と引き続き整合する。実装計画ではこの結論
   （移設先・移設範囲・元ファイルの扱い）をそのまま採用してよい。
...
3. **`expiry-alert-subscription.tsx` の初期表示時の購読状態（ON/OFF どちらを表示するか）の
   ...
   現在の購読状態を取得して初期表示に反映するかどうかは記載が無い。設計書 §将来課題に
   「実装時に確定し、テストで固定する対象」として明記されたため、本書の EAS-09（要確認）
   観点はそのまま残す。
4. **`PushSubscription.createdAt` getter が防御的コピーを返すかどうかが設計書に明記されて
   ...
   設計書 §将来課題に「実装時に確定し、テストで固定する対象」として明記されたため、本書の
   PS-08（要確認）観点はそのまま残す。
...
7. **`stock-row.tsx` のクライアントバンドルへの影響（R-12）は、設計書自身が「確認推奨」
   ...
   P-14 により `formatExpiryUrgencyLabel` も同じ import 元に加わるため、確認対象の関数が
   2 個から 3 個に増える点も申し送りに含める。
