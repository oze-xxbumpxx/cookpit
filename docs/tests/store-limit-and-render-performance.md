...
| ID   | 対象                   | 手順                                                           | 合否基準                                     |
| ---- | ---------------------- | -------------------------------------------------------------- | -------------------------------------------- |
| M-01 | `loading.tsx` 6 ルート | dev サーバーで各ルートへ遷移し、応答前に骨組みが出るか確認     | 遷移直後に骨組みが描画される                 |
| M-02 | フォントの見た目       | 主要 6 画面を差し替え前後で見比べる                            | 丸ゴシックのままで崩れがない                 |
| M-03 | サブセット取りこぼし   | 商品名・レシピ名に第一水準の漢字を入力して表示                 | fallback フォントに落ちない                  |
| M-04 | 転送量                 | 生成された woff2 3 本の合計サイズを測る                        | 4.40 MB → 1.0 MB 未満                        |
| M-05 | 店舗の上限運用         | 本番相当データで 10 件 → 3 件まで削除し、その後 1 件追加を試す | 3 件までは削除でき、2 件以下で追加が再開する |

実施手順は `manual-browser-verify` スキルに従い、項目ごとに PASS / BLOCKED(理由) / FAIL を報告する。
