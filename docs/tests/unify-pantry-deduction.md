...
| U-5  | `reconcileItemDeduction`: 増加・pantry 無し                                                                                 | 買う量 2・sunk `null`、生必要量 4、pantry 在庫 0                                                                                                                                   | 同上                                                                 | `action:'update'`、`amount:{value:4}`、`pantryDeductedAmount:null`（値なしは null。0 の Quantity は書かない）、`consumed:false`                     | 正常系           |
...
| U-8  | `reconcileItemDeduction`: `item.productId === null`（P-2 対象外・数量あり材料）                                             | 増加ケース（生必要量 4、sunk `null`）、`item.productId: null`、pantry に在庫があっても productId 不一致で消費対象外                                                                | 同上                                                                 | `action:'update'`、`amount` は受け取った `additionalNeeded` そのまま（4）、`pantryDeductedAmount:null`、`consumed:false`、pantry 不変               | 正常系（防御的） |
...
| U-10 | `reconcileItemDeduction`: 集計に一致行が無い（呼び出し側前提の破れ）                                                        | Orchestrator 確定: throw しない。`itemMatchKey` が productId 優先のため「item の productId が null で集計行が非 null」は同一キーでは通常到達不能。到達可能な防御は aggregated 欠落 | 空の Map で直接呼ぶ                                                  | `action:'none'`、`consumed:false`、`covered:null`。例外は投げない。productId 不一致の極端な組は対象外（照合キー規則上到達不能）                     | 異常系（防御性） |
...
可算単位で生必要量に端数がある材料（例: 必要 4.5 個・在庫 1 個）は、Generate が切り上げて買う量 4 になる一方、部分引き算後の再同期は残り必要量を在庫 0 の `applyPantryDeduction` に渡すため切り上げが効かず買う量が 3.5 になりうる。切り上げは在庫があるときだけ適用する既存仕様（D-5 の実装）に由来する。在庫なし Generate の買う量まで変えないため本タスクでは直さない。Done ロック I-11〜I-14 は整数必要量で一致を固定する。
