...
>
> **M-1 修正後の再計測（2026-08-07）**: `736.*.js` **373 KB** / `3957.*.js` 5 KB、
> どちらも client-reference-manifest 参照 **0 件**。`price-history-chart.tsx` から
> skeleton ファイルへの import を足したが、skeleton は recharts に依存しないため
> 分離は維持されている。
...
| MB-05 | **価格記録 0 件の商品**でレイアウトシフトが発生しない（レビュー M-1 で追加） | 価格記録を持たない商品を 1 件シード（例: にんじん） | 「価格推移」セクションの高さが初期表示から変わらないこと。**スケルトン（`h-64` = 256px）を経由せず、空状態文言（約 86px）が最初から描画される**こと。直下の「この商品を削除」ボタンの Y 座標が動かないこと |
