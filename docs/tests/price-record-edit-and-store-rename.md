...
- **防御性（Domain 層）**:
  ...
  - 不正引数の伝搬: D-PUR-06（price=0）、D-PUR-07（packageSize=0）、D-SRN-02/03
    （空文字・空白のみ）。
- **申し送り（2026-08-06 の実画面確認で判明）**: **編集で内容量の単位を変えると、単価表示の
  分母（基準）も一緒に変わる。** 表示基準は記録時の単位から決まる仕様
  （`product-format.ts` の `unitPriceBasis`。`kg`/`l` は `1kg`/`1L` 基準、`g`/`ml` は
  `100g`/`100ml` 基準）で、編集前後で単位の次元が変わると
  「1,000円 / 1kg」→「200円 / 100g」のように**分母が変わって直感的に比較できない**。
  DB の正準値（100g / 100ml あたり）は一貫しており店舗間比較は壊れないため、
  これは表示の読み取りやすさの課題であり不具合ではない。編集機能が入って初めて
  「同じ記録の単位を変える」経路が生まれたため、ここに記録しておく。
...
> **実施結果（2026-08-06）: MB-01〜05 全 PASS。** PGlite dev + Playwright / 390px。
> 実施時に判明した本節の誤りを 2 点是正した。
>
> 1. **MB-03 の前提記述が誤っていた**。「編集前の単価表示は 100円 / 100g」と書いていたが、
>    `kg` 記録の単価表示は `100g` 基準ではなく **`1kg` 基準**（`product-format.ts` の
>    `WEIGHT_BASIS` が `kg` に `factor: 10` / `label: '1kg'` を割り当てる）。実際の編集前表示は
>    **「1,000円 / 1kg」**である。編集後（500g）が「200円 / 100g」になるのは正しい
>    （1000円/1kg = 100円/100g、1000円/500g = 200円/100g）。
>    → **編集で単位を変えると単価表示の分母（基準）も変わる**点は §11 の申し送りに追記した。
> 2. **MB-04 の前提は「店舗A に価格記録 2 件」を満たすシードが必要**。単純に商品を増やすと
>    同一店舗に記録が集まり件数が変わるため、確認用シードでは 6 件の記録を別店舗へ寄せた。
>
> また、確認手順自体の落とし穴として: **PGlite の dev DB を再シードするときは dev サーバーを
> 止めてから行う**こと。起動中に `.pglite-dev` を作り直すと、サーバーが削除済みディレクトリの
> ハンドルを掴んだまま古い状態を返し続け、「編集前の前提を満たしていないのに PASS に見える」
> 状態になる（実際に一度これで誤判定した）。
...
| MB-03 | kg→g への単位変更編集で、画面上の単価表示が再計算後の値に変わる                          | 価格記録 1 件（店舗A・`price: 1000円`・`packageSize: 1kg`。編集前の単価表示は **「1,000円 / 1kg」**。1kg 記録は 1kg 基準で表示される）をシード済み。編集ダイアログで `packageSize` を `500g` に変更して保存する | 保存後、「最近の記録」の該当行の単価表示が**「200円 / 100g」に変わっており**（編集前の「1,000円 / 1kg」のまま残っていないことを明示的に確認する）、内容量表示が「500g」になっていること                                                                                                               |
