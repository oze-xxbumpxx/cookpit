...
- 作成日: 2026-08-08
- 前提となる設計書: `docs/designs/stock-edit.md`（P-1〜P-6 ユーザー確定・2026-08-07・confirmed）
...
| --- | ----------------- | -------------- | ----------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------- |
| S-1 | 牛乳              | fridge         | asOf + 1 日                         | 閾値内（critical/soon 相当） | 緊急度チップ表示の確認（MB-03）                                                       |
| S-2 | 冷凍餃子          | freezer        | asOf + 20 日                        | 閾値外                       | 緊急度チップ**非表示**の確認（MB-04。閾値外にチップが出ないことの固定）               |
| S-3 | 米                | pantry         | null（未設定）                      | 該当なし                     | 期限なし在庫の表示（プレーン表示・チップ無し）の確認（MB-05）                         |
| S-4 | 卵                | null（未設定） | asOf - 1 日（**期限切れ**）         | overdue                      | 「期限切れ」ラベルの確認・保存場所「未設定」表示の確認（MB-06）                       |
| S-5 | 玉ねぎ            | fridge         | asOf 当日                           | critical（本日まで）         | 境界値「本日まで」表示の確認（MB-07）                                                 |
| S-6 | 味噌              | pantry         | asOf + 3 日（**閾値ちょうど**）     | 閾値内の境界                 | 「3 日以内」の境界（含む）の確認（MB-08。閾値超えとの誤判定が無いことの直接比較材料） |
| S-7 | 塩                | freezer        | asOf + 4 日（**閾値超えの最小差**） | 閾値外の境界                 | S-6 との対比でチップの有無が閾値 3/4 日で正しく切り替わることの確認（MB-08）          |

- 数量編集の確認（MB-01/MB-02）には S-1（単位変更を伴う編集）・S-3（値のみ変更）の 2 件を使う。
- 買い物完了パネルの確認（MB-09〜11）には、シード済みデータとは別に**買い物リストの品目**を
