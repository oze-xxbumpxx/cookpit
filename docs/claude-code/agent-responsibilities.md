...
> **2026-07-25 / IMP-2026-030**: 低稼働 4 Agent を統合し **11 Agent** 構成へ。凍結定義は
> [archive/agents/](./archive/agents/)。security-reviewer は単独維持。

## 一覧

| Agent                     | 役割                                                           | 主な出力                                                | ツール                                          | コード変更   |
| ------------------------- | -------------------------------------------------------------- | ------------------------------------------------------- | ----------------------------------------------- | ------------ |
| orchestrator              | 指揮・委譲・統合・要求分析                                     | 委譲計画・統合報告・要求メモ                            | Agent, Read, Grep, Glob                         | しない       |
| architecture-designer     | 技術設計（+ 条件付きパフォーマンス / L3 要件書 Write）         | `docs/designs/<feature>.md`（+ requirements）           | Read, Grep, Glob, Write                         | しない       |
...
| test-designer             | 試験観点・試験計画（+ 条件付き E2E 実装）                      | `docs/tests/<feature>.md`（+ E2E テスト）               | Read, Grep, Glob, Edit, Write, Bash             | テストのみ\* |
...
> ツール権限は最小限に絞る。`Agent`（Subagent 起動）を持つのは **orchestrator**（全実務 Agent）と
> **agent-improvement-manager**（回帰評価の agent-evaluator のみ）の 2 つに限る。
> プロダクションコードを変更するのは **implementer** のみ。
> \*test-designer のコード変更は、Orchestrator が明示した **結合/E2E テストのみ**
> （プロダクションコード不可）。設計・計画・試験計画・振り返りは Write で `docs/` 配下のみ。
> 改善系 3 Agent の責務詳細は [improvement-cycle.md](./improvement-cycle.md) を参照。

## orchestrator（要求分析を含む）

- ユーザー要求の整理、不明点・制約・前提条件の抽出（旧 requirements-analyst）。
- 既存コード・既存仕様の調査、影響範囲の整理、試験観点の洗い出し。
- 変更レベル判定・委譲・統合・完了条件確認。
- **コードは変更しない。** L3 の要件書ファイル保存は architecture-designer へ指示する。

## architecture-designer

- 技術設計、レイヤーと責務、API/DB/フロント/バックエンド設計、データフロー、
  エラー処理、ログと監視、セキュリティ、性能、後方互換性、テスト方針。
- **条件付き（L3・外部 I/O / 大量データ / 性能要件）**: 旧 performance-designer の観点
  （N+1・インデックス・キャッシュ・RT 予算・負荷シナリオ）を設計書のパフォーマンス節へ。
- L3 では Orchestrator 指示により `docs/requirements/<feature>.md` を保存し、
  L2/L3 の最初の Write 担当として `.claude/state/current-feature` を書く。
...
- **条件付き（L3・E2E 基盤整備済み・implementer 完了後）**: 旧 e2e-test-implementer の
  結合/E2E テスト実装。プロダクションコードは変更しない。
- 試験計画フェーズではコードを変更しない。
...
- **文書レビュー**（旧 document-reviewer）: 事実整合・矛盾・参照生存・鮮度・完全性・
  読者適合・主張の強度・表記一貫性。開発フロー成果物にもフロー外文書にも使える。
- 原則コード・文書を変更せず、指摘と修正案を提示。保存が必要なら `docs/reviews/<feature>.md`。
- 仕様の事実確認は自身の Read/Grep で行う。変更を伴う再依頼は Orchestrator へ差し戻す。
...
- `reviewer` と役割を分担: reviewer は品質/整合性/文書を担当、security-reviewer はセキュリティ深掘り。
