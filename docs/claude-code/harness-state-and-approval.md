...
4. **GitHub Actions が実行されていない（期限付きの既知リスク・2026-08 に解消予定）**:
   ...
   **待機期間中のリスク姿勢（重要）**: この間、安全境界は**ローカルの
   ...
   このためローカル層の健全性が待機期間中の唯一の防御になる。再監査で発見した
   保護境界の欠陥（R-001 ほか 3 件）は **2026-07-26 に修正適用済み**（`4cd9774`。
   詳細は [harness-owner-setup.md](./harness-owner-setup.md) §0）。
...
6. **保護境界を修復する手段が環境内に存在しない（2026-07-26 の再監査で発見）**:
   ...
   （実際に 2026-07-26 の再監査でロックアウトが発生し、修正は検証済みパッチとして
   リポジトリへ保全したうえで、人間が自分の端末で適用するしかなかった）。
