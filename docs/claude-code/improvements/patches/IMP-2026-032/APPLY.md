...
- **適用**: **未適用**。`.claude/agents/` は保護対象であり、承認の発行は人間しかできない。
  リモートセッションの Claude は `harness-approve.mjs` を実行できず（`guard-dangerous.mjs` が deny）、
  当該コンテナに TTY も `HARNESS_APPROVAL_TOKEN` も無いため、承認を発行する経路が存在しない。
- **同期文書**: 適用済み（`improvement-cycle.md` §リモートセッションでの承認手順のフォールバック
  「保護ファイル分をローカル適用待ちとして backlog へ記録し、docs 側のみ適用する」に従う）。
