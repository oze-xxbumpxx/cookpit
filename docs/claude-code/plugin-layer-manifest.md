...
## 割り当て（`.claude/` 全 74 ファイル）

`state/`（gitignore 対象）・`evals/results/`・`evals/baselines/`（実績値）・個人設定の
`settings.local.json` は配布対象外のため計上しない。
...
| `scripts/check-review-coverage.mjs`            | **improvement から再分類**。Task review の網羅確認 |
| `scripts/review-readiness.mjs`                 | review subject / packet / CI 診断                  |
| `tests/review-readiness.test.mjs`              | review workflow の回帰                             |
...
ディレクトリ別に層へ割り当てた件数（2026-08-11 実測・全 74 ファイル）。
...
`state/`（gitignore 対象）・`evals/results/`・`evals/baselines/`（実績値）・
`settings.local.json` は配布対象外のため計上しない。**未分類 0 件。**

検算コマンド:

```bash
find .claude -type f ! -path "*/state/*" ! -path "*/evals/results/*" \
  ! -path "*/evals/baselines/*" ! -name "settings.local.json" | wc -l # → 74
```
...
grep -rlE "collect-task-metrics|record-task-metrics|sprint-summary|check-review-coverage|review-readiness|session-briefing" \
...
grep -rlE "collect-task-metrics|record-task-metrics|sprint-summary|reflect-task|reflection-agent" \
