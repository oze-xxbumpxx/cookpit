...
- Agent、Hook、Rule、Skill、`CLAUDE.md` などの重要構成は、作業ブランチへコミットし、
  **人間による PR レビューとマージ判断**を承認境界にする。
...
5. PR に変更範囲、検証結果、ロールバック方法を残す。
6. ユーザーが差分を確認し、マージを決定する。

`main` へ直接反映しない。承認は `.claude/state/` のマーカーではなく、PR のレビューと
マージ履歴で追跡する。
