...
| Agent                     | Model    | 役割                                             | 起動条件                                      |
| ------------------------- | -------- | ------------------------------------------------ | --------------------------------------------- |
| orchestrator              | opus-4-8 | 指揮・委譲・統合・要求分析                       | 複数工程の開発タスク                          |
| architecture-designer     | sonnet-5 | 技術設計（+ 条件付きパフォーマンス / L3 要件書） | L2/L3                                         |
| contract-designer         | sonnet-5 | 契約設計（Zod/Drizzle/Hono RPC/DTO）             | 契約変更があるとき                            |
| test-designer             | sonnet-5 | 試験観点・試験計画（+ 条件付き E2E 実装）        | L2/L3（E2E は L3・基盤整備済みの再委譲）      |
| implementation-planner    | sonnet-5 | 実装計画                                         | L2/L3                                         |
| implementer               | sonnet-5 | 実装・単体テスト・品質ゲート                     | L1〜L3                                        |
| reviewer                  | opus-4-8 | 独立レビュー（コード + 文書）                    | L2/L3（文書レビュー依頼時も）                 |
