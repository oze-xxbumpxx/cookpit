...
| Agent | モデル | 役割 | 変更権限 |
| --- | --- | --- | --- |
| reflection-agent | sonnet-4-6 | 振り返り・候補抽出 | 設定変更不可（candidates のみ Write） |
| agent-improvement-manager | opus-4-8 | 横断分析・提案・軽微反映 | 重要設定は不可（提案のみ）。軽微は proposal 付きで可 |
| agent-evaluator | sonnet-4-6 | 回帰評価 | 変更不可（evaluations のみ Write） |
...
- 重要設定の変更は単一コミットにまとめ、戻しやすくする（commit/push は明示指示時のみ）。
