...
（出典: IMP-2026-029 — 承認済み変更が適用できないデッドロック累計 3 回の解消）:
...
> マーカー作成・削除コマンドは `.claude/settings.json` の `permissions.allow` へ登録する
> （権限クラシファイアの自動拒否対策。差分は proposals/IMP-2026-029.md — settings.json 自体が
> 保護ファイルのため初回登録のみローカル適用）。登録前・または環境側で拒否された場合は、
> 保護ファイル分を「ローカル適用待ち」として backlog へ記録し、docs 側のみ適用する
> （従来運用へフォールバック）。
