...
かつては承認ファイル（`.claude/state/config-change-approved` と `harness-approve.mjs`）で
Hook が機械的に強制していたが、2026-07-28 に撤去した。理由は 2 つ:

1. 個人開発では承認する人とされる人が同一で、儀式が実質的な検証になっていない。
2. 承認ファイルは実行中コンテナ内に置かれるため、リモートのエフェメラル環境からは
   発行できず、**ハーネスが自分自身の修正を恒久的に拒否する**デッドロックを生んでいた
   （IMP-2026-029 で 3 回再発。2026-07-27 に再々発を実証）。

> 注意: PR レビューを境界にする以上、その実効性は branch protection と CI に依存する。
> GitHub Actions が課金停止で不動作の間、実効的な境界は「マージ前の目視」のみになる
> （harness-owner-setup.md §branch protection）。
...
- 重要設定（構成ファイル）の変更は単一コミットにまとめ、戻しやすくする。
