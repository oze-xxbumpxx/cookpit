...
> これらは承認前には proposal と差分の提示だけにとどめる。`PostToolUse` の
> `validate-agent-config` Hook は構文と整合性を検証するが、人間承認の有無は判定しない。
> 承認の記録は proposal の「採用または却下理由」と PR の履歴に残す。

### 重要構成を適用する手順（PR レビュー）

1. proposal または変更案で、対象、差分、リスク、ロールバック方法を提示する。
2. ユーザーがチャットで**対象範囲を明示承認**する。
3. 専用の作業ブランチで、承認された差分だけを適用する。
4. `pnpm test:harness` と対象に応じた品質ゲートを通す。
5. Claude Code のレビューを受け、重大な指摘を解消する。
6. PR に承認の出典、変更範囲、検証結果を記載する。
7. ユーザーが PR の差分を確認し、マージを決定する。
...
- 重要設定の変更は単一コミットまたは単一 PR にまとめ、戻しやすくする。コミットは人間の
  明示承認後に行う（指定の作業ブランチへの通常コミット/push は CLAUDE.md 行動制約に従い可）。
