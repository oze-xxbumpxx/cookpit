...
- **Phase 3 の骨子起案の前に Sprint 11（安定化と答え合わせ・1週間）を挟む**（ユーザー確定・2026-08-17）。下記 §Sprint 11 を参照
...
## Sprint 11：安定化と答え合わせ（1週間・Phase 3 への橋渡し・2026-08-17 ユーザー確定）

MVP2 クローズ後、**Phase 3 の骨子起案の前に 1 本挟む**（ユーザー確定・2026-08-17）。
MVP2 の 4 スプリントには属さず、Phase 3 のスプリント番号にも含めない橋渡しスプリント。

### 置く理由

- **Phase 3 の上位候補は着手条件が未達**。月次の食費分析 / 食材ロス分析は
  「価格記録・消費履歴の蓄積が前提」（下記 §MVP2 に含めないもの）。残る候補は
  MVP2 が守った優先軸「実運用の穴 > 新機能 > 基盤」の外側にある
- **MVP2 最大の学びへの手当てが未着手**。[sprint10-review-2026-08-17.md](sprints/sprint10-review-2026-08-17.md)
  §次スプリント候補の「高」3 件はいずれも Sprint 10 の本番障害 3 回の後始末。Phase 3 候補は
  LLM 提案・OCR と**外部 I/O を増やす方向**のため、先に閉じておく
- **MVP2 のゴールの答え合わせが未実施**。2 人で土曜運用を 1 周する観察を通すと、Phase 3 の入力が
  「候補リスト」から「実運用で出た困りごと」に変わる。MVP2 の入力が MVP1 の ADR・設計書の
  将来課題だったのと同じ作り方になる

### ゴール

**Phase 3 の骨子を、決着した基盤と実運用の観察の上で起案できる状態にする。**

### タスク

| #   | タスク                                                                                                            | 出典・補足                                                                                              |
| --- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 1   | U-2：`DB_WRITE_TRANSACTION` ON/OFF の書き込みレイテンシを実測                                                     | 継続課題 1 / `docs/reviews/uow.md` R-08。見積もり 10〜30ms の裏取り                                     |
| 2   | U-3 / U-4：本番 `DATABASE_URL` の pooled 判定と transaction pooling の制約確認                                    | 継続課題 2 / R-09                                                                                       |
| 3   | WebSocket 書き込み経路の実行時自動テスト（+ `createTxConnection` に成功ログ 1 行）                                | 継続課題 3・4。現状 PGlite のみで、依存更新時は Preview の手動一巡が必要。本番障害 3 回の再発防止に直結 |
| 4   | 土曜運用 1 周の観察。観点リスト・合否基準・記録先を `docs/tests/saturday-operation.md` に定義**してから**実施する | MVP2 ゴールの答え合わせ / MVP1 からの積み残し。定義なしで実施すると結果がログに戻らない                 |
| 5   | 計測・レビュー習慣の仕組み化（`TASK-*.yml` の記録漏れ / 週次スプリントレビューの Routine 化）                     | レトロ改善点 2〜4。Sprint 5 レビュー継続課題 6 が未実施のまま Sprint 10 で悪化                          |
| 6   | 余力：`shopping-list-client.offline-queue.test.tsx` のフレーキー修正                                              | 継続課題 6。**完了条件に含めない**                                                                      |
| 7   | 余力：改善候補 4 件（`uow` / `offline-write-queue` / `expiry-alert` / `meal-plan-sync`）の昇格判断                | 継続課題 7。**完了条件に含めない**                                                                      |

**対象外**：タスク 6「`ConsumeStock` の冪等性キー」は契約変更を伴う L2〜L3 のため本スプリントに
載せず、**Phase 3 の先頭候補**として残す（オフラインキューを `consume` へ広げる先行条件）。

### 完了条件

Sprint 6 の反証不能な完了条件（「2人で1週間の運用を完走」）を繰り返さないため、
**「穴が無いことの証明」ではなく「実施と記録の有無」**で切る。

- [ ] `DB_WRITE_TRANSACTION` ON/OFF の書き込みレイテンシ差分が実測値として文書に残っている
- [ ] 本番 `DATABASE_URL` の pooled 判定と transaction pooling の制約が文書に残り、U-3 / U-4 が未決事項から外れている
- [ ] WebSocket 書き込み経路が自動テストで検証され、依存更新時の Preview 手動一巡が不要になっている
- [ ] 土曜運用 1 周を観点リストつきで実施し、結果（PASS / FAIL / 気づき）が記録先に残っている
- [ ] 週次スプリントレビューが仕組み（Routine 等）で起動する状態になっている

### 進め方

- 変更レベルは着手時に宣言する。タスク 1・2・4 は L1〜L2、タスク 3・5 は L2 想定
- タスク 4 の観点リストは、MVP2 クローズ節が挙げた 2 クラス（自由記述データ起因の表示崩れ /
  機能の継ぎ目）を必ず含める
- 本スプリント終了時に **Phase 3 の骨子（ゴール・優先軸・スプリント分割）を起案**し、
  ユーザー確定を取ってから Phase 3 へ移行する
