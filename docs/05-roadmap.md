...
| 4   | 小粒 UX（`useLeaveConfirmation` 適用・steps 並べ替え） | 2026-07-25 からの継続。**レビュー中**（`cursor/small-ux-c4d2` / PR #167）                                                                    |
...
- [x] 献立からレシピを外すと買い物リストが追随する（Unit A・PR #162。
      `docs/tests/meal-plan-sync.md` の完了条件は実機確認を要求していない）
- [ ] 集約横断の書き込みが部分失敗しない（トランザクション境界が引かれている）
      — **PR #164 で一度達成したが PR #168 で差し戻し。本番は非トランザクション**（後述）
...
| Unit B: uow                     | タスク 3（DB トランザクション / UoW）+ Sprint 9 申し送りの `DrizzleShoppingListRepository.save()` 3 往復削減                                   | **L3**               | Orchestrator 経路                    | **再着手が要る**（PR #164 マージ → PR #168 で本番ドライバを差し戻し。構造は残るが本番の原子性は無効）   |
...
| サイド: small-ux / フォント判断 | タスク 4 / タスク 7                                                                                                                            | L2 / L0              | Orchestrator（4）/ ユーザー判断（7） | **完了**（タスク 4・PR #167 マージ）/ 余力（タスク 7）                                                  |
...
#### Sprint 10 の完了可否判断（2026-08-14・ユーザー依頼により判断）

**Sprint 10 は締めない。開けたまま残作業を進める**（ユーザー確定）。完了条件 3 件のうち達成は 1 件。

| 完了条件                     | 判定                | 実態                                                                                                          |
| ---------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------- |
| 献立の変更が追随する         | ✅ 達成             | Unit A（PR #162）。実装 + 自動テスト。試験計画の完了条件は実機確認を要求していない                            |
| 集約横断の書き込みが原子的   | ❌ 達成後に差し戻し | PR #164 → **PR #168**。本番は `neon-http` + `useTransaction: false` で `execute` が `work()` の恒等実行になる |
| 期限の実機確認（Sprint 8 3） | ❌ 未着手           | 本番 `VAPID_PUBLIC_KEY` 未設定。Sprint 8 から 2 スプリント連続で停止                                          |

**条件 2 を「構造は入っている」で達成扱いにしない。** Domain の `UnitOfWork` ポートと書き込み
UseCase 30 本の `execute` 包みはコードに残っているが、本番で原子性は成立していない。これは
Sprint 8 の完了条件 3 件目で「実装が乗っていることと成立していることは別」として完了印を付けず、
Sprint 9 クローズで Sprint 6 の反証不能な完了条件と対比して立てた基準と、そのまま同じ形である。
`docs/reviews/meal-plan-sync.md` が残存リスクに挙げた「ShoppingList 保存成功後の Pantry 保存失敗」
の窓も、Unit B で閉じる前提だったのに開いたままになっている。

**Unit B 再着手の前提**（差し戻し時の原因記述は切り分け未了。ADR-0019 §実行記録を参照）:
差し戻し前のコードは `neonConfig.webSocketConstructor` を設定済みで、初歩的な設定漏れではない。
同じ変更を再適用せず、`ws` のバンドル・`globalThis` の `Pool` キャッシュ・WebSocket 非到達の
3 候補を Preview で切り分けてから方式を決める。**Preview 検証を工程に組み込むまで本番へ入れない**
（Unit B レビュー H-01 の推奨 reject を飛ばしたことが前回の障害の原因）。

**クローズの条件**: 条件 2 が Preview 検証込みで本番有効になり、条件 3 の MB-01〜14 が実機 PASS。

**未確認のまま残っている事実**: PR #168 マージ後の本番 `GET /api/health` が `db: "connected"` に
戻ったかの記録が無い（セッション 9 の申し送り）。リモート環境は外部 egress が遮断されており
Claude 側からは確認できない。
