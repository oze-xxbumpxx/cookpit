...
- [ ] オフライン中のチェック操作が復帰後に反映される（実装・自動テスト済。Chromium 代替確認は実施。iPhone PWA 実機はマージ後）
- [ ] Function を `sin1` へ移したあと、本番 curl で DB 1 往復が 0.20 秒から縮むことを確認（フォント実測はタスク 4 繰り越しに伴い本条件から外す）
