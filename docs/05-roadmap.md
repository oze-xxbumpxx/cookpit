...
**骨子（未確定）**。Sprint 6 の完了条件（2人で1週間の運用を完走）を先に締め、そこで出た
課題を Sprint 7 の先頭へ差し込んでから確定する（ユーザー確定・2026-07-29）。
...
| Sprint    | 期間    | スコープ                             | 主な入力                    |
| --------- | ------- | ------------------------------------ | --------------------------- |
| Sprint 7  | 1.5週間 | 価格比較の完成                       | 困りごと 1 の未達分         |
...
| #   | タスク                                         | 出典・補足                                                                                      |
| --- | ---------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| 1   | 価格差の定量表示                               | `docs/designs/shopping-list-screens.md` S-6（案 B/C。バルク取得 API の要否を設計で判断）        |
| 2   | 価格記録の編集                                 | 2026-07-27 積み残し（現状は「削除して記録し直す」のみ）                                         |
| 3   | 店舗のリネーム（`PUT /api/stores/:id`）        | 2026-07-27 積み残し（ADR-0012 で受容したトレードオフ。ADR-0013 のカスケード削除で緊急度は低下） |
| 4   | 商品詳細の DB 往復削減 + recharts 遅延読み込み | 2026-07-27 積み残し（`findById` が 2 回・重複）                                                 |

**完了条件**

- [ ] 買い物リストで「どちらの店舗が何円安いか」が画面で分かる
...
**期限の入力・編集手段（タスク 1・2）がアラート（タスク 3）の前提**。契約・DTO・UseCase は
すでに `expiresAt` を受け取れる形なので、穴は UI 側に閉じている。
