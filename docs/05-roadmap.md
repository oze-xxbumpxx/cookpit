...
| 2   | Vercel Function を hnd1（東京）へ移す判断                                           | Neon は `ap-southeast-1`（シンガポール、`logs/sprint-0.md`）。本番 Function は `iad1`（`x-vercel-id`）。同居前提は不成立。Preview で `hnd1` を A/B 測定中。本番確定は測定後。`sin1`（Neon と同リージョン）も候補 |
