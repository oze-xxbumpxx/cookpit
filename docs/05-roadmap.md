...
**実施結果（2026-08-15）**: 完了条件 3 件目は達成した。本番に VAPID 3 点 + `CRON_SECRET` を入れ、
...
（Preview の Deployment Protection が iPhone PWA を阻むため）。試験計画 MB-01〜14 の全文と
Android 対照は未消化。詳細は `logs/2026-08-15.md`。
...
| 5   | **Sprint 8 Unit B（expiry-alert）の実機確認**          | **完了条件は達成**（2026-08-15 iPhone PWA。本番 Cron 手動実行で `sentCount: 1`）。試験計画 MB 全文と Android 対照は未消化 |
...
| Unit B: uow                     | タスク 3（DB トランザクション / UoW）+ Sprint 9 申し送りの `DrizzleShoppingListRepository.save()` 3 往復削減                                   | **L3**               | Orchestrator 経路                    | **構造は #164。本番は #168 で neon-http に差し戻し。対話型 TX は未達**      |
