...
| Unit B: uow                     | タスク 3（DB トランザクション / UoW）+ Sprint 9 申し送りの `DrizzleShoppingListRepository.save()` 3 往復削減                                   | **L3**               | Orchestrator 経路                    | 構造は #164。本番は #168 で neon-http に差し戻し。**案 S を実装済み・main 未マージ・有効化未確認** |
