...
| Unit B: uow                     | タスク 3（DB トランザクション / UoW）+ Sprint 9 申し送りの `DrizzleShoppingListRepository.save()` 3 往復削減                                   | **L3**               | Orchestrator 経路                    | **レビュー中**（ADR-0019。neon-serverless + UseCase が `uow.execute`）                                  |
