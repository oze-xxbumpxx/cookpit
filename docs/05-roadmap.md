...
- ~~認証導入（Better Auth）~~ — [ADR-0023](./decisions/ADR-0023-better-auth-login.md) で
  2026-09-17 に導入済み（email + password + Cookie セッション。ADR-0021 の Basic 認証を置換）。
  複数ユーザー対応（ロール・3 人目以降の運用）は引き続き対象外
...
| ~~認証（Better Auth）~~                 | [ADR-0023](./decisions/ADR-0023-better-auth-login.md) で 2026-09-17 に導入済み                     |
| 友人との献立シェア                      | 2 人運用では不要のまま                                                                             |
