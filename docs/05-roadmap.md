...
| Sprint 10 | 1.5週間 | 献立の精度 + 横断基盤 **着手済み**（2026-08-13）   | ADR の将来課題              |
...
- [x] 期限が近い在庫にアプリを開かずに気づける — Unit B（PR #147）。**2026-08-15 に
      iPhone 実機（ホーム画面追加済み PWA）で Push 受信とタップ遷移を確認して達成**
      （`docs/tests/expiry-alert.md` §10-5。MB-08 / MB-09 PASS）。Android 対照
      （MB-08b 等 5 項目）は端末が無く未実施のまま残す
...
| Unit B: expiry-alert | タスク 3（賞味期限アラート）                                                                                                                           | L3     | Orchestrator 経路 | **完了**（成果物 7 点 + ADR-0017 + 全層の実装。PR #147）。実機 Push 受信は **2026-08-15 に iPhone で確認**（§10-5。Android 対照のみ未実施） |
...
**実機確認を実施し、完了条件 3 件目を達成として閉じた。** ユーザーの iPhone
（ホーム画面追加済み PWA）で Cron 手動トリガー後に通知を受信し、タップで `/pantry` へ
遷移することを確認した。結果の全量は `docs/tests/expiry-alert.md` §10-5。

| 区分                                                                    | 結果                                                          |
| ----------------------------------------------------------------------- | ------------------------------------------------------------- |
| iOS 側（MB-03 / 03b / 04 / 05 / 06 / 07 / 08 / 09 / 10 / 11 / 13 / 14） | **12 項目 PASS**                                              |
| Android 側（MB-01 / 02 / 08b / 09b / 12）                               | **未実施**（Android 実機を用意できず。PS-1 が未シードのまま） |

**完了印を付けた理由**: Sprint 8 の完了条件は「期限が近い在庫にアプリを開かずに気づける」で、
利用者の実機で通知を受け取れた時点で成立している。2026-08-10 に完了印を保留した理由
（「実機で一度も通知を受けていない」）は解消した。

**それでも閉じきっていないもの**: 試験計画が必須に指定している **Android 対照（MB-08b）は
未取得**。`VAPID_SUBJECT` の形式ミスのように片側 OS だけ落ちる障害（罠 8・R-9）の検出経路が
空いたままである。

**未実施分は追わない（2026-08-15 ユーザー判断）。** Android 5 項目（MB-01 / 02 / 08b /
09b / 12）は Sprint 10 以降の持ち越しタスクとして起票せず、`docs/tests/expiry-alert.md`
§10-5 を最終記録として閉じる。主対象が iPhone PWA で Android は現在の利用形態に含まれない
ため。Android 対応や複数デバイス利用が要件に入った時点で再訪する。
...
- 本番 `VAPID_PUBLIC_KEY` 未設定（Sprint 8 持ち越し。vapid プローブは 500 のまま。通知は購読
  ON まで飛ばない）
...
| 4   | 小粒 UX（`useLeaveConfirmation` 適用・steps 並べ替え） | **完了**（PR #167 マージ）。2026-07-25 からの継続                                                                                        |
| 5   | **Sprint 8 Unit B（expiry-alert）の実機確認**          | **完了**（2026-08-15）。`docs/tests/expiry-alert.md` §10-5。iPhone で 12 項目 PASS。Android 対照 5 項目（MB-01/02/08b/09b/12）のみ未実施 |
...
| Unit B: uow                     | タスク 3（DB トランザクション / UoW）+ Sprint 9 申し送りの `DrizzleShoppingListRepository.save()` 3 往復削減                                   | **L3**               | Orchestrator 経路                    | **構造のみ完了**（PR #164・ADR-0019）。PR #168 で本番の対話型 tx を差し戻し済み（`useTransaction: false`）。再導入案は `docs/designs/uow.md` |
...
| サイド: expiry-alert 実機       | タスク 5                                                                                                                                       | **L0**               | ユーザー実機 + Preview               | **完了**（2026-08-15）。完了条件 3 件目を達成。MB-04（vapid 200）を含む iOS 12 項目 PASS / Android 5 項目未実施                              |
| サイド: small-ux / フォント判断 | タスク 4 / タスク 7                                                                                                                            | L2 / L0              | Orchestrator（4）/ ユーザー判断（7） | **完了**（タスク 4・PR #167）/ 余力（タスク 7）                                                                                              |
