...
### 認証は Better Auth（email + password + Cookie セッション）
...
公開に先立って `apps/web/src/proxy.ts`（Next.js 16 の `proxy` 規約。Node.js ランタイム）による
Basic 認証へ一時的に移行した（[ADR-0021](./decisions/ADR-0021-basic-auth-for-public-repository.md)。
Superseded）。
