...
> 詳細画面（`/recipes/[id]`、`tasks/sprint1-recipe-ui-detail.md`）が**この指針より後に実装される場合**は、詳細画面は最初から本トークン（`bg-background` / `text-foreground` / `bg-primary` 等）で実装すること。先に実装済みなら同じ移行マッピングを適用する。
