...
const targetRoot = path.resolve(
  targetIndex >= 0 ? args[targetIndex + 1] : process.cwd(),
);
if (targetIndex >= 0 && !args[targetIndex + 1])
  throw new Error('--target requires a path');

const manifest = JSON.parse(
  await readFile(path.join(pluginRoot, 'install-manifest.json'), 'utf8'),
);
...
for (const file of files) {
  ...
  if (await exists(destination) && !(await same(source, destination)))
    conflicts.push(file);
}
if (conflicts.length && !force) {
  ...
  console.error(
    'Review them, then rerun with --force only if replacement is intended.',
  );
  process.exit(2);
}
...
for (const [event, entries] of Object.entries(manifest.hooks ?? {})) {
  ...
  for (const entry of entries)
    if (!seen.has(JSON.stringify(entry))) current.push(entry);
...
console.log(
  `Installed ${manifest.name}: ${files.length} files into ${targetRoot}`,
);
